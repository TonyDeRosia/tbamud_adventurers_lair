...
static void medit_disp_ai_threat_sequence(struct descriptor_data *d) { struct mob_ai_config*c=OLC_MOB(d)->ai_config;int i;write_to_output(d,"\r\nThreat Escalation Sequence\r\n");for(i=0;i<c->threat_step_count;i++)write_to_output(d,"%d) %s Severity %d Cooldown %d Repeats %d Advance %s\r\n",i+1,threat_name(c->threat_steps[i].type),c->threat_steps[i].minimum_severity,c->threat_steps[i].cooldown,c->threat_steps[i].max_repetitions,c->threat_steps[i].advance_on_failure?"Yes":"No");write_to_output(d,"A <type severity cooldown repeats advance>; E <line type severity cooldown repeats advance>; D <line>; U <line>; N <line>; Q) Return\r\nChoice: ");OLC_MODE(d)=MEDIT_AI_THREAT_SEQUENCE; }
static void medit_disp_ai_combat(struct descriptor_data *d) { struct mob_ai_config*c=OLC_MOB(d)->ai_config; write_to_output(d,"\r\nAI Actor Combat\r\n1) Style:%d 2) Policy:%s 3) Initiate:%s 4) Assist:%s 5) Call help:%s 6) Flee:%s\r\n7) Protect trusted:%s 8) Protect group:%s 9) Same role:%s A) Same prototype:%s\r\nB) Retaliate self:%s C) Retaliate ally:%s D) Retaliate hostile:%s E) Switch targets:%s\r\nF) Flee HP:%d%% G) Assist severity:%d H) Switch threshold:%d I) Cooldown:%d\r\nJ) Target preferences K) Preview L) Validate Q) Return\r\nChoice: ",c->combat_style,c->combat_enabled?"Yes":"No",c->may_initiate?"Yes":"No",c->may_assist?"Yes":"No",c->may_call_help?"Yes":"No",c->may_flee?"Yes":"No",c->protect_trusted?"Yes":"No",c->protect_group?"Yes":"No",c->protect_same_role?"Yes":"No",c->protect_same_prototype?"Yes":"No",c->retaliate_self?"Yes":"No",c->retaliate_ally?"Yes":"No",c->retaliate_hostile?"Yes":"No",c->switch_targets?"Yes":"No",c->flee_hp_percent,c->assist_severity,c->target_switch_threshold,c->combat_cooldown); OLC_MODE(d)=MEDIT_AI_COMBAT; }
static void medit_disp_ai_targets(struct descriptor_data *d) { static const char *names[] = { "Current attacker", "Attacker of trusted actor", "Attacker of group member", "Known hostile", "Lowest health / closest to death", "Player character", "NPC", "Previous target" }; struct mob_ai_config*c=OLC_MOB(d)->ai_config; int i; write_to_output(d,"\r\nTarget Preferences (supported, signed -100..100)\r\n"); for(i=0;i<AI_TARGET_WEIGHTS;i++) write_to_output(d,"%d) %-35s %d\r\n",i+1,names[i],c->target_weight[i]); write_to_output(d,"Unavailable: spellcaster, healer, ranged attacker, criminal, feared actor, highest damage output.\r\nQ) Return\r\nChoice: "); OLC_MODE(d)=MEDIT_AI_TARGETS; }
...
static const char *medit_schedule_room_name(int vnum)
{ room_rnum room=real_room(vnum); return room==NOWHERE ? "Invalid/Unset" : world[room].name; }
static void medit_disp_ai_schedule(struct descriptor_data *d) { struct mob_ai_config*c=OLC_MOB(d)->ai_config; int i;
 write_to_output(d,"\r\nAI Actor Schedule\r\n1) Enabled:%s  R) Resume:%s  F) Default failure:%d\r\n2) Home:%d (%s)\r\n3) Work:%d (%s)\r\n4) Sleep:%d (%s)\r\n5) Guard:%d (%s)\r\n6) Fallback:%d (%s)\r\n",c->schedule_enabled?"Configured":"Unset",c->resume_after_interrupt?"Yes":"No",c->default_failure_policy,c->home_room_vnum,medit_schedule_room_name(c->home_room_vnum),c->work_room_vnum,medit_schedule_room_name(c->work_room_vnum),c->sleep_room_vnum,medit_schedule_room_name(c->sleep_room_vnum),c->guard_room_vnum,medit_schedule_room_name(c->guard_room_vnum),c->fallback_room_vnum,medit_schedule_room_name(c->fallback_room_vnum));
 write_to_output(d,"Entries (%d/%d):\r\n",c->schedule_count,AI_SCHEDULE_MAX); for(i=0;i<c->schedule_count;i++)write_to_output(d," %d) id %d %s %02d-%02d days %02x priority %d activity %d destination %d/%d\r\n",i+1,c->schedules[i].id,c->schedules[i].enabled?"Enabled":"Disabled",c->schedules[i].start_hour,c->schedules[i].end_hour,c->schedules[i].day_mask,c->schedules[i].priority,c->schedules[i].activity,c->schedules[i].destination,c->schedules[i].destination_value);
 write_to_output(d,"A) Add  E <n field value>) Edit  T <n>) Toggle  D/U/N/X <n>) Delete/Move/Duplicate\r\nR) Resume toggle  F <policy>) Default failure  2-6 <room|clear>) locations\r\nP [day hour]) Preview  V) Validate  Routes: Y list | Y A | Y D <n> | Y X <n> | Y W <route> <room> <wait>\r\nQ) Return\r\nChoice: ");OLC_MODE(d)=MEDIT_AI_SCHEDULE; }
...
void medit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case MEDIT_AI_COMBAT: { struct mob_ai_config*c=OLC_MOB(d)->ai_config; char report[MAX_STRING_LENGTH]; if(LOWER(*arg)=='q'){medit_disp_ai_menu(d);return;} if(LOWER(*arg)=='j'){medit_disp_ai_targets(d);return;} if(LOWER(*arg)=='k'){ai_actor_combat_preview(c,report,sizeof(report));write_to_output(d,"\r\n%s\r\n",report);medit_disp_ai_combat(d);return;} if(LOWER(*arg)=='l'){ai_actor_combat_validate(c,report,sizeof(report));write_to_output(d,"\r\n%s\r\n",report);medit_disp_ai_combat(d);return;} i=(LOWER(*arg)>='a'&&LOWER(*arg)<='j')?10+LOWER(*arg)-'a':atoi(arg); if(i>=2&&i<=14){int *v=&c->combat_enabled+i-2;*v=!*v;OLC_VAL(d)=1;medit_disp_ai_combat(d);return;} if(i==1||i>=15&&i<=18){OLC_VAL(d)=i;OLC_MODE(d)=MEDIT_AI_COMBAT_VALUE;write_to_output(d,"Value: ");return;}medit_disp_ai_combat(d);return;}
  case MEDIT_AI_COMBAT_VALUE: {struct mob_ai_config*c=OLC_MOB(d)->ai_config;int *v=OLC_VAL(d)==1?&c->combat_style:OLC_VAL(d)==15?&c->flee_hp_percent:OLC_VAL(d)==16?&c->assist_severity:OLC_VAL(d)==17?&c->target_switch_threshold:&c->combat_cooldown;if(i<0||(OLC_VAL(d)==1&&i>=AI_COMBAT_STYLE_MAX)||(OLC_VAL(d)>1&&OLC_VAL(d)<18&&i>100)||(OLC_VAL(d)==18&&i>300)){write_to_output(d,"Invalid value: ");return;}*v=i;OLC_VAL(d)=1;medit_disp_ai_combat(d);return;}
  ...
  case MEDIT_AI_SCHEDULE: { struct mob_ai_config*c=OLC_MOB(d)->ai_config; char cmd=LOWER(*arg), field[32]; int n=0,v=0,room; char report[MAX_STRING_LENGTH];
    if(cmd=='q'){medit_disp_ai_menu(d);return;}
    if(cmd>='2'&&cmd<='6'){ if(!str_cmp(arg+1,"clear"))v=0; else if(sscanf(arg+1,"%d",&v)!=1||(v&&real_room(v)==NOWHERE)){write_to_output(d,"Room must be an existing VNUM or clear.\r\n");medit_disp_ai_schedule(d);return;} if(cmd=='2')c->home_room_vnum=v;else if(cmd=='3')c->work_room_vnum=v;else if(cmd=='4')c->sleep_room_vnum=v;else if(cmd=='5')c->guard_room_vnum=v;else c->fallback_room_vnum=v;OLC_VAL(d)=1;medit_disp_ai_schedule(d);return; }
    if(cmd=='e'&&sscanf(arg+1,"%d %31s %d",&n,field,&v)==3&&--n>=0&&n<c->schedule_count){struct ai_schedule_entry *e=&c->schedules[n];int ok=TRUE;if(!str_cmp(field,"enabled"))e->enabled=!!v;else if(!str_cmp(field,"start"))e->start_hour=v;else if(!str_cmp(field,"end"))e->end_hour=v;else if(!str_cmp(field,"days"))e->day_mask=v;else if(!str_cmp(field,"priority"))e->priority=v;else if(!str_cmp(field,"activity"))e->activity=v;else if(!str_cmp(field,"destination"))e->destination=v;else if(!str_cmp(field,"value"))e->destination_value=v;else if(!str_cmp(field,"arrival"))e->arrival_action=v;else if(!str_cmp(field,"departure"))e->departure_action=v;else if(!str_cmp(field,"interrupt"))e->interruption_policy=v;else if(!str_cmp(field,"failure"))e->failure_policy=v;else if(!str_cmp(field,"travel"))e->max_travel_time=v;else if(!str_cmp(field,"attempts"))e->max_attempts=v;else if(!str_cmp(field,"wait"))e->wait_duration=v;else if(!str_cmp(field,"route"))e->route_id=v;else ok=FALSE; if(e->start_hour<0||e->start_hour>23||e->end_hour<0||e->end_hour>23||e->day_mask<1||e->day_mask>AI_DAY_MASK_ALL||e->priority<-100||e->priority>100||e->activity<0||e->activity>=AI_SCHEDULE_ACTIVITY_MAX||e->destination<0||e->destination>=AI_DESTINATION_MAX||e->arrival_action<0||e->arrival_action>=AI_SCHEDULE_ACTION_MAX||e->departure_action<0||e->departure_action>=AI_SCHEDULE_ACTION_MAX||e->interruption_policy<0||e->interruption_policy>=AI_INTERRUPT_MAX||e->failure_policy<0||e->failure_policy>=AI_FAILURE_MAX||e->max_attempts<1||e->max_attempts>100||e->max_travel_time<0||e->max_travel_time>3600||e->wait_duration<0||e->wait_duration>3600)ok=FALSE; if(!ok){write_to_output(d,"Invalid field/value; entry left unchanged is required—edit rejected.\r\n");}else OLC_VAL(d)=1;medit_disp_ai_schedule(d);return; }
    if(cmd=='y'){char sub=LOWER(arg[1]);if(sub=='a'){struct ai_patrol_route r;memset(&r,0,sizeof(r));r.enabled=TRUE;r.loop_mode=AI_PATROL_LOOP;if(ai_patrol_add(c,&r))OLC_VAL(d)=1;}else if(sub=='d'&&sscanf(arg+2,"%d",&n)==1&&n>0&&n<=c->patrol_count){memmove(&c->patrols[n-1],&c->patrols[n],sizeof(c->patrols[0])*(c->patrol_count-n));c->patrol_count--;OLC_VAL(d)=1;}else if(sub=='x'&&sscanf(arg+2,"%d",&n)==1&&n>0&&n<=c->patrol_count){struct ai_patrol_route r=c->patrols[n-1];r.id=0;if(ai_patrol_add(c,&r))OLC_VAL(d)=1;}else if(sub=='w'&&sscanf(arg+2,"%d %d %d",&n,&room,&v)==3&&n>0&&n<=c->patrol_count&&real_room(room)!=NOWHERE&&c->patrols[n-1].waypoint_count<AI_PATROL_WAYPOINT_MAX){struct ai_patrol_waypoint *w=&c->patrols[n-1].waypoints[c->patrols[n-1].waypoint_count++];memset(w,0,sizeof(*w));w->room_vnum=room;w->wait_duration=AI_MEDIT_CLAMP(v,0,3600);OLC_VAL(d)=1;}medit_disp_ai_schedule(d);return;}
    if(cmd=='1'){c->schedule_enabled=!c->schedule_enabled;OLC_VAL(d)=1;}else if(cmd=='r'){c->resume_after_interrupt=!c->resume_after_interrupt;OLC_VAL(d)=1;}else if(cmd=='f'&&sscanf(arg+1,"%d",&v)==1&&v>=0&&v<AI_FAILURE_MAX){c->default_failure_policy=v;OLC_VAL(d)=1;}else if(cmd=='a'){struct ai_schedule_entry e;memset(&e,0,sizeof(e));e.enabled=TRUE;e.start_hour=e.end_hour=0;e.day_mask=AI_DAY_MASK_ALL;e.activity=AI_SCHEDULE_REMAIN;e.destination=AI_DEST_CURRENT_ROOM;e.max_attempts=3;if(ai_schedule_add(c,&e))OLC_VAL(d)=1;}else if(cmd=='t'&&sscanf(arg+1,"%d",&n)==1&&n>0&&n<=c->schedule_count){c->schedules[n-1].enabled=!c->schedules[n-1].enabled;OLC_VAL(d)=1;}else if(cmd=='d'&&ai_schedule_delete(c,atoi(arg+1)-1))OLC_VAL(d)=1;else if(cmd=='u'&&ai_schedule_move(c,atoi(arg+1)-1,atoi(arg+1)-2))OLC_VAL(d)=1;else if(cmd=='n'&&ai_schedule_move(c,atoi(arg+1)-1,atoi(arg+1)))OLC_VAL(d)=1;else if(cmd=='x'&&ai_schedule_duplicate(c,atoi(arg+1)-1))OLC_VAL(d)=1;else if(cmd=='p'){int day=((35*time_info.month)+(time_info.day+1))%7,hour=time_info.hours;sscanf(arg+1,"%d %d",&day,&hour);ai_actor_schedule_preview(c,day,hour,report,sizeof(report));write_to_output(d,"%s",report);}else if(cmd=='v'){ai_actor_schedule_validate(c,report,sizeof(report));write_to_output(d,"%s",report);}medit_disp_ai_schedule(d);return; }

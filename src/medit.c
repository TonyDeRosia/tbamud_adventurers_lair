...
void medit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case MEDIT_AI_COMBAT: { struct mob_ai_config*c=OLC_MOB(d)->ai_config; if(LOWER(*arg)=='q'){medit_disp_ai_menu(d);return;} if(LOWER(*arg)=='j'){medit_disp_ai_targets(d);return;} i=(LOWER(*arg)>='a'&&LOWER(*arg)<='j')?10+LOWER(*arg)-'a':atoi(arg); if(i>=2&&i<=14){int *v=&c->combat_enabled+i-2;*v=!*v;OLC_VAL(d)=1;medit_disp_ai_combat(d);return;} if(i==1||i>=15&&i<=18){OLC_VAL(d)=i;OLC_MODE(d)=MEDIT_AI_COMBAT_VALUE;write_to_output(d,"Value: ");return;}medit_disp_ai_combat(d);return;}

...
static void medit_disp_behavior_objects(struct descriptor_data *d);
...
static const char *medit_validate_routine_room(struct descriptor_data *d, int vnum, int random_index, const struct ai_schedule_entry *editing);
...
static void medit_disp_ai_vocalization_entry(struct descriptor_data *d);
static const char *medit_ai_vocal_presence_name(int presence);
static const char *medit_ai_vocal_frequency_name(int frequency);
...
static const char *medit_ai_vocal_presence_name(int presence) { static const char *names[] = { "No presence requirement", "At least one player", "At least one awake player", "At least one visible player", "Any character" }; return names[MAX(0, MIN(presence, 4))]; }
static const char *medit_ai_vocal_frequency_name(int frequency) { static const char *names[] = { "Very rare", "Rare", "Occasionally", "Often", "Very often" }; return names[MAX(1, MIN(frequency, 5)) - 1]; }
static void medit_ai_vocal_prompt(struct descriptor_data *d) { write_to_output(d, "Enter the room message for this creature sound.\r\nExamples:\r\n  Slurrrrpp!\r\n  The green gelatinous blob gurgles wetly.\r\nQ cancels.\r\nCreature sound: "); }
static const char *medit_validate_creature_sound(char *arg)
{ char *start, *end, *p; if (!arg) return "Creature sound cannot be empty."; start=arg; while (*start && isspace((unsigned char)*start)) start++; end=start+strlen(start); while(end>start&&isspace((unsigned char)end[-1])) *--end='\0'; if(start!=arg) memmove(arg,start,strlen(start)+1); if(!*arg) return "Creature sound cannot be empty."; if(strlen(arg)>=AI_VOCALIZATION_LINE_MAX) return "Creature sound is too long. Maximum length is 200 characters."; for(p=arg;*p;p++) if((unsigned char)*p<32||(unsigned char)*p==127) return "Creature sound contains unsupported control characters."; return NULL; }
...
static void medit_disp_ai_communication(struct descriptor_data *d)
{
  ...
  if (c->communication == AI_COMM_VOCALIZE) {
    snprintf(limit, sizeof(limit), c->vocal_room_limit ? "%d sounds" : "Unlimited", c->vocal_room_limit);
    write_to_output(d, "4) Edit Creature Sounds\r\n5) Presence Requirement : %s\r\n6) Frequency            : %s\r\n7) Cooldown             : %d-%d seconds\r\n8) Per-room Limit       : %s\r\n9) Preview Sounds\r\n", medit_ai_vocal_presence_name(c->vocal_presence), medit_ai_vocal_frequency_name(c->vocal_frequency), c->vocal_cooldown_min, c->vocal_cooldown_max, limit);
    if (!c->vocalization_count || c->vocal_disabled_mask == ((1U << c->vocalization_count) - 1U)) write_to_output(d, "\r\nWarning:\r\n    Creature Sounds is enabled, but no active sounds are authored.\r\n");
  }
...
{
  write_to_output(d, "\r\nAdvanced Behavior\r\n-----------------\r\n\r\n1) Personality and Social Style\r\n2) Perception and Awareness\r\n3) Memory Details\r\n4) Threat Response\r\n5) Combat Reactions\r\n6) Assistance Rules\r\n7) Capability Overrides\r\n8) Movement Internals\r\n9) Profile and Inference\r\nP) What this NPC will do (technical)\r\nV) Validation Information\r\nD) Technical Diagnostics\r\nR) Reset Inferred Defaults\r\n\r\nH) Help\r\nQ) Return\r\nChoice: ");
  OLC_MODE(d) = MEDIT_AI_ADVANCED;
}
static void medit_disp_ai_diagnostics(struct descriptor_data *d)
{
  struct mob_ai_config *c = OLC_MOB(d)->ai_config;
  char sources[MAX_STRING_LENGTH];
  int spoken = 0, i;
  for (i = 0; i < AI_DIALOGUE_CATEGORIES; i++) spoken += c->dialogue_count[i];
  write_to_output(d, "\r\nDiagnostics & Sources\r\n---------------------\r\n\r\nBehavior Sources\r\n  Movement : %s%s\r\n  Routine  : %s\r\n  Combat   : %s%s\r\n  Memory   : %s%s\r\n  Objects  : %s\r\n  DG Scripts: %s\r\n  Unknown Special: %s\r\n\r\n",
    MOB_FLAGGED(OLC_MOB(d),MOB_SENTINEL)?"NPC Flags; ":"",MOB_FLAGGED(OLC_MOB(d),MOB_AI_ACTOR)?"Behavior configuration":"mobile activity",
    GET_MOB_SPEC(OLC_MOB(d))?(legacy_special_metadata(GET_MOB_SPEC(OLC_MOB(d)))?legacy_special_metadata(GET_MOB_SPEC(OLC_MOB(d)))->name:"custom special"):(c->schedule_count||c->patrol_count?"Behavior configuration":"None"),
    MOB_FLAGGED(OLC_MOB(d),MOB_AGGRESSIVE)||MOB_FLAGGED(OLC_MOB(d),MOB_HELPER)?"NPC Flags; ":"",MOB_FLAGGED(OLC_MOB(d),MOB_AI_ACTOR)?"Behavior configuration":"mobile activity",
    MOB_FLAGGED(OLC_MOB(d),MOB_MEMORY)?"NPC Flag; ":"",c->memory_enabled?"Relationship memory":"None",
    MOB_FLAGGED(OLC_MOB(d),MOB_SCAVENGER)?"NPC Flag":"None",OLC_MOB(d)->proto_script?"Present":"None",GET_MOB_SPEC(OLC_MOB(d))&&!legacy_special_metadata(GET_MOB_SPEC(OLC_MOB(d)))?"Yes":"No");
  ...
  legacy_behavior_summary(OLC_MOB(d),sources,sizeof(sources),FALSE);
  write_to_output(d,"Technical source detail\r\n%s\r\n",sources);
  write_to_output(d, "Warnings: %d\r\n\r\nP) Preview Effective Behavior\r\nD) Advanced Technical Diagnostics\r\nH) Help\r\nQ) Return\r\nChoice: ",ai_actor_compatibility_warning_count(OLC_MOB(d)));
...
static int medit_owner_reject(struct descriptor_data *d, unsigned domain, enum mob_behavior_owner owner, char *why, size_t why_size)
{
  ...
  if (owner == MOB_BEHAVIOR_OWNER_AI && (domain == LBD_SCAVENGING || domain == LBD_FLEE)) { snprintf(why, why_size, "AI ownership unavailable: NPC behavior implementation for this domain is not complete."); return TRUE; }
  return FALSE;
}
static void medit_disp_ai_ownership_value(struct descriptor_data *d)
{
  int idx = medit_owner_load(d); unsigned domain = (idx >= 0 && idx < MOB_BEHAVIOR_EDITABLE_DOMAIN_COUNT) ? mob_behavior_editable_domain(idx) : LBD_MOVEMENT; int di = mob_behavior_domain_index(domain); struct mob_ai_config *c = OLC_MOB(d)->ai_config; char why[128]="";
...
static void medit_disp_ai_ownership(struct descriptor_data *d)
{
  ...
  for (i=0;i<MOB_BEHAVIOR_EDITABLE_DOMAIN_COUNT;i++) { int di=mob_behavior_domain_index(mob_behavior_editable_domain(i)); write_to_output(d, " %d) %-32s %s\r\n", i+1, medit_owner_domain_label(mob_behavior_editable_domain(i)), mob_behavior_owner_name(c->behavior_owner[di])); }
...
static void medit_disp_ai_personality(struct descriptor_data *d)
{
  ...
  write_to_output(d, "\r\nNPC behavior Personality (0-100)\r\n%s\r\n", medit_ai_personality_summary(c));
...
static void medit_disp_ai_routine_random(struct descriptor_data *d) { struct mob_ai_config*c=OLC_MOB(d)->ai_config; int i; write_to_output(d,"\r\nRandom Destination Travel\r\n-------------------------\r\n\r\nDestinations:\r\n\r\n"); for(i=0;i<c->random_destination_count;i++){room_rnum r=real_room(c->random_destinations[i].room_vnum);write_to_output(d,"%d) [%s] %d - %s\r\n",i+1,c->random_destinations[i].enabled?"Enabled":"Disabled",c->random_destinations[i].room_vnum,r==NOWHERE?"INVALID":world[r].name);} if(c->routine_mode==AI_ROUTINE_NONE) write_to_output(d,"\r\nNote: destinations are inactive until a routine mode including Random Destination Travel is enabled.\r\n"); write_to_output(d,"\r\nWait After Arrival:\r\n    %d-%d seconds\r\n\r\nSelection:\r\n    Random\r\n\r\nA) Add Destination\r\nW) Change Wait Time\r\nP) Preview\r\nV) Validate All\r\nQ) Return\r\nChoice: ",c->random_wait_min,c->random_wait_max);OLC_MODE(d)=MEDIT_AI_ROUTINE_RANDOM; }
static void medit_disp_ai_routine_random_entry(struct descriptor_data*d){struct mob_ai_config*c=OLC_MOB(d)->ai_config;int n=OLC_VAL(d),r;if(n<0||n>=c->random_destination_count){medit_disp_ai_routine_random(d);return;}r=real_room(c->random_destinations[n].room_vnum);write_to_output(d,"\r\nRandom Destination\r\n------------------\r\n\r\nRoom:\r\n    %d - %s\r\n\r\nStatus:\r\n    %s\r\n\r\n1) Change Room VNUM\r\n2) Enable/Disable\r\n5) Preview Path\r\n6) Validate\r\nD) Delete\r\nQ) Return\r\nChoice: ",c->random_destinations[n].room_vnum,r==NOWHERE?"INVALID":world[r].name,c->random_destinations[n].enabled?"Enabled":"Disabled");OLC_MODE(d)=MEDIT_AI_ROUTINE_RANDOM_ENTRY;}
static void medit_disp_ai_schedule(struct descriptor_data *d) { struct mob_ai_config*c=OLC_MOB(d)->ai_config; const char *m[]={"No Routine","Random Destination Travel","Scheduled Destinations","Random + Scheduled","Advanced Patrol"}; int i, random_enabled=0, scheduled_enabled=0; for(i=0;i<c->random_destination_count;i++) random_enabled+=c->random_destinations[i].enabled; for(i=0;i<c->schedule_count;i++) scheduled_enabled+=c->schedules[i].enabled; write_to_output(d,"\r\nDaily Routine\r\n-------------\r\n\r\nMode:\r\n    %s\r\n\r\nRandom Destinations:\r\n    %d enabled (%d authored)\r\n\r\nScheduled Destinations:\r\n    %d enabled (%d authored)\r\n\r\nTravel Boundary:\r\n    %s\r\n\r\nWait After Random Arrival:\r\n    %d-%d seconds\r\n\r\n1) Change Routine Mode\r\n2) Random Destinations\r\n3) Scheduled Destinations\r\n4) Travel Settings\r\n5) Preview Routine\r\n6) Validate Routine\r\n\r\nA) Advanced Patrol Editor\r\nH) Help\r\nQ) Return\r\nChoice: ",m[MAX(0,MIN(c->routine_mode,4))],random_enabled,c->random_destination_count,scheduled_enabled,c->schedule_count,c->routine_boundary==AI_ROUTINE_CURRENT_ZONE?"Current Zone":"Allowed Zones",c->random_wait_min,c->random_wait_max); OLC_MODE(d)=MEDIT_AI_SCHEDULE; }
static void medit_disp_ai_schedule_entries(struct descriptor_data *d) { struct mob_ai_config*c=OLC_MOB(d)->ai_config;int i;write_to_output(d,"\r\n%s                    Scheduled Destinations%s\r\n\r\n #  State     Time       Days        Priority  Activity\r\n",CCCYN(d->character,C_NRM),CCNRM(d->character,C_NRM));for(i=0;i<c->schedule_count;i++){struct ai_schedule_entry*e=&c->schedules[i];write_to_output(d," %d  %-8s  %-9s %-11s %-9d %s\r\n",i+1,e->enabled?"Enabled":"Disabled",e->start_hour==0&&e->end_hour==0?"All Day":"game time",medit_sched_days(e->day_mask),e->priority,medit_sched_activity(e->activity));}write_to_output(d,"\r\nCommands\r\n  A) Add Entry  E) Edit Entry  D) Delete Entry  U) Move Entry Up\r\n  N) Move Entry Down  C) Duplicate Entry  T) Enable/Disable Entry\r\n  H) Help  Q) Return\r\nChoice: ");OLC_MODE(d)=MEDIT_AI_SCHEDULE_ENTRIES; }
...
static void medit_disp_behavior_objects(struct descriptor_data *d)
{
  write_to_output(d,"\r\nObject Interaction\r\n------------------\r\n1) Pick up valuable objects : %s\r\n2) Equipment, carried items and loot drops\r\n\r\nPickup, drop and future object interactions not supported by the current runtime are shown here as Locked rather than hidden.\r\n3) General pickup rules      : Locked (not implemented)\r\n4) Deliberate drop rules     : Locked (not implemented)\r\nQ) Return\r\nChoice: ",MOB_FLAGGED(OLC_MOB(d),MOB_SCAVENGER)?"Yes":"No");
  OLC_MODE(d)=MEDIT_BEHAVIOR_OBJECTS;
}
...
static void medit_disp_ai_vocalizations(struct descriptor_data *d)
{
  ...
  if (!c->vocalization_count)
    write_to_output(d, "\r\nNo creature sounds have been authored.\r\n\r\nExamples:\r\n  Slurrrrpp!\r\n  The green gelatinous blob gurgles wetly.\r\n");
  for (i = 0; i < c->vocalization_count; i++)
    write_to_output(d, "  %d) %s%s\r\n", i + 1, (c->vocal_disabled_mask&(1U<<i)) ? "[disabled] " : "", c->vocalization[i]);
  if (!c->vocalization_count) write_to_output(d, "\r\nA) Add Sound\r\nP) Preview\r\nH) Help\r\nQ) Return\r\nChoice: ");
  else if (c->vocalization_count == 1) write_to_output(d, "\r\nEnter 1 to edit the sound.\r\nA) Add Sound\r\nP) Preview\r\nH) Help\r\nQ) Return\r\nChoice: ");
  else write_to_output(d, "\r\nEnter a sound number to edit it.\r\nA) Add Sound\r\nP) Preview\r\nH) Help\r\nQ) Return\r\nChoice: ");
  OLC_MODE(d) = MEDIT_AI_VOCALIZATIONS;
}
static void medit_disp_ai_vocalization_entry(struct descriptor_data *d)
{ struct mob_ai_config *c=OLC_MOB(d)->ai_config; int n=OLC_VAL(d); if(n<0||n>=c->vocalization_count){medit_disp_ai_vocalizations(d);return;} write_to_output(d,"\r\nCreature Sound %d\r\n----------------\r\n\r\nSound Text:\r\n  %s\r\n\r\n1) Change Text\r\n2) %s\r\n3) Duplicate\r\n",n+1,c->vocalization[n],(c->vocal_disabled_mask&(1U<<n))?"Enable":"Disable"); if(n>0)write_to_output(d,"4) Move Up\r\n"); if(n+1<c->vocalization_count)write_to_output(d,"5) Move Down\r\n"); write_to_output(d,"D) Delete\r\nQ) Return\r\nChoice: "); OLC_MODE(d)=MEDIT_AI_VOCALIZATION_INDEX; }
...
static int medit_is_ai_mode(int mode)
{
  ...
  case MEDIT_AI_TRAIT:
  case MEDIT_AI_PRESET:
  case MEDIT_AI_SOCIAL:
  case MEDIT_AI_SOCIAL_VALUE:
  case MEDIT_AI_DIALOGUE:
  case MEDIT_AI_DIALOGUE_ADD:
  case MEDIT_AI_DIALOGUE_EDIT:
  case MEDIT_AI_DIALOGUE_DELETE:
  case MEDIT_AI_DIALOGUE_REORDER:
  case MEDIT_AI_ENABLE_CONFIRM:
  case MEDIT_AI_PERCEPTION:
  case MEDIT_AI_PERCEPTION_VALUE:
  case MEDIT_AI_MEMORY:
  case MEDIT_AI_MEMORY_VALUE:
  case MEDIT_AI_THREAT:
  case MEDIT_AI_THREAT_VALUE:
  case MEDIT_AI_THREAT_SEQUENCE:
  case MEDIT_AI_THREAT_STEP:
  case MEDIT_AI_COMBAT:
  case MEDIT_AI_COMBAT_VALUE:
  case MEDIT_AI_TARGETS:
  case MEDIT_AI_TARGET_VALUE:
  case MEDIT_AI_SCHEDULE:
  case MEDIT_AI_SCHEDULE_EDIT:
  case MEDIT_AI_SCHEDULE_ROOM:
  case MEDIT_AI_SCHEDULE_FAILURE:
  case MEDIT_AI_SCHEDULE_ENTRIES:
  case MEDIT_AI_SCHEDULE_ENTRY:
  case MEDIT_AI_SCHEDULE_ENTRY_VALUE:
  case MEDIT_AI_SCHEDULE_DAYS:
  case MEDIT_AI_SCHEDULE_ACTIVITY:
  case MEDIT_AI_SCHEDULE_DESTINATION:
  case MEDIT_AI_SCHEDULE_ROUTE:
  case MEDIT_AI_SCHEDULE_ACTION:
  case MEDIT_AI_SCHEDULE_INTERRUPT:
  case MEDIT_AI_SCHEDULE_DIALOGUE:
  case MEDIT_AI_SCHEDULE_DELETE:
  case MEDIT_AI_PATROL_ROUTES:
  case MEDIT_AI_PATROL_ROUTE:
  case MEDIT_AI_PATROL_LABEL:
  case MEDIT_AI_PATROL_MODE:
  case MEDIT_AI_PATROL_FAILURE:
  case MEDIT_AI_PATROL_DELETE:
  case MEDIT_AI_PATROL_WAYPOINTS:
  case MEDIT_AI_PATROL_WAYPOINT:
  case MEDIT_AI_DIALOGUE_INDEX:
  case MEDIT_AI_SCHEDULE_INDEX:
  case MEDIT_AI_HELP:
  case MEDIT_AI_COMPATIBILITY:
  case MEDIT_AI_CAPABILITIES:
  case MEDIT_AI_CAPABILITY_VALUE:
  case MEDIT_AI_VOCALIZATIONS:
  case MEDIT_AI_VOCALIZATION_ADD:
  case MEDIT_AI_VOCALIZATION_EDIT:
  case MEDIT_AI_VOCALIZATION_INDEX:
  case MEDIT_AI_PATROL_WAYPOINT_VALUE:
  case MEDIT_AI_PATROL_WAYPOINT_ACTION:
  case MEDIT_AI_PATROL_WAYPOINT_DELETE:
  case MEDIT_AI_ADVANCED:
  case MEDIT_AI_COMMUNICATION:
  case MEDIT_AI_INTELLIGENCE:
  case MEDIT_AI_DIAGNOSTICS:
  case MEDIT_AI_VOCAL_PRESENCE:
  case MEDIT_AI_VOCAL_FREQUENCY:
  case MEDIT_AI_VOCAL_COOLDOWN:
  case MEDIT_AI_VOCAL_LIMIT:
  case MEDIT_AI_OWNERSHIP:
  case MEDIT_AI_OWNERSHIP_VALUE:
  case MEDIT_AI_OWNERSHIP_RESET:
  case MEDIT_AI_ROUTINE_RANDOM:
  case MEDIT_AI_ROUTINE_RANDOM_ENTRY:
  case MEDIT_AI_ROUTINE_RANDOM_VNUM:
  case MEDIT_AI_ROUTINE_WAIT:
  case MEDIT_AI_ROUTINE_MODE:
  case MEDIT_AI_ROUTINE_TIMING:
...
static const char *medit_validate_routine_room(struct descriptor_data *d, int vnum,
                                               int random_index,
                                               const struct ai_schedule_entry *editing)
{
  struct mob_ai_config *c = OLC_MOB(d)->ai_config;
  room_rnum room;
  int i, mob_zone;

  if (vnum <= 0) return "Enter a numeric room VNUM.";
  room = real_room(vnum);
  if (room == NOWHERE) return "That room VNUM does not exist.";
  if (ROOM_FLAGGED(room, ROOM_NOMOB)) return "That room is flagged ROOM_NOMOB.";
  if (ROOM_FLAGGED(room, ROOM_DEATH)) return "That room is a death room.";
  mob_zone = real_zone_by_thing(OLC_NUM(d));
  if ((MOB_FLAGGED(OLC_MOB(d), MOB_STAY_ZONE) ||
       c->routine_boundary == AI_ROUTINE_CURRENT_ZONE) &&
      mob_zone != NOWHERE && world[room].zone != mob_zone)
    return "That room is outside this NPC's allowed zone.";
  if (!editing) {
    for (i = 0; i < c->random_destination_count; i++)
      if (i != random_index && c->random_destinations[i].room_vnum == vnum)
        return "That random destination is already configured.";
  } else {
    for (i = 0; i < c->schedule_count; i++)
      if (&c->schedules[i] != editing &&
          c->schedules[i].destination == AI_DEST_ROOM_VNUM &&
          c->schedules[i].destination_value == vnum)
        return "That scheduled destination is already configured.";
  }
  return NULL;
}
...
static int medit_parse_ai_integer(const char *arg, int minimum, int maximum, int *value)
{
  char *end;
  long parsed;
  if (!arg || !*arg || !isdigit((unsigned char)*arg))
    return FALSE;
  errno = 0;
...
void medit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case MEDIT_AI_DIAGNOSTICS:
    if (LOWER(*arg) == 'q') { medit_disp_ai_menu(d); return; }
    if (LOWER(*arg) == 'h') { medit_disp_ai_help(d, MEDIT_AI_DIAGNOSTICS, "Diagnostics", "Diagnostics lists only issues a builder can act on, such as inactive lines, schedules, and movement restrictions.", "Use Preview to understand effective behavior.", "Technical runtime detail is available only in Advanced Behavior."); return; }
  ...
  case MEDIT_AI_OWNERSHIP_VALUE: {
    ...
    domain=mob_behavior_editable_domain(idx); di=mob_behavior_domain_index(domain); owner=(enum mob_behavior_owner)(i-1);
  ...
  case MEDIT_AI_OWNERSHIP_RESET:
    ...
    if (LOWER(*arg)=='y') { int k; if (!OLC_MOB(d)->ai_config) OLC_MOB(d)->ai_config=mob_ai_config_new(); for(k=0;k<MOB_BEHAVIOR_DOMAIN_COUNT;k++) OLC_MOB(d)->ai_config->behavior_owner[k]=MOB_BEHAVIOR_OWNER_COMPATIBILITY; OLC_VAL(d)=1; medit_disp_ai_ownership(d); return; }
  ...
  case MEDIT_AI_COMMUNICATION:
    ...
    if (LOWER(*arg) == '9' && c->communication == AI_COMM_VOCALIZE) { int active=0; write_to_output(d, "\r\nCreature Sounds Preview\r\n-----------------------\r\n\r\n"); for (i=0; i<c->vocalization_count; i++) if (!(c->vocal_disabled_mask&(1U<<i))) { if(!active++)write_to_output(d,"Players may hear:\r\n\r\n"); write_to_output(d,"  %s\r\n\r\n",c->vocalization[i]); } if(!active)write_to_output(d,"No creature sounds have been authored.\r\n\r\nThis NPC will remain silent even though its Communication style is set to\r\nCreature Sounds.\r\n\r\nUse \"Edit Creature Sounds\" and choose \"Add Sound.\"\r\n\r\nExamples:\r\n  Slurrrrpp!\r\n  The green gelatinous blob gurgles wetly.\r\n"); write_to_output(d,"Presence requirement:\r\n  %s\r\nFrequency:\r\n  %s\r\nCooldown:\r\n  %d-%d seconds\r\nPer-room limit:\r\n  %s\r\n",medit_ai_vocal_presence_name(c->vocal_presence),medit_ai_vocal_frequency_name(c->vocal_frequency),c->vocal_cooldown_min,c->vocal_cooldown_max,c->vocal_room_limit?"Configured":"Unlimited"); medit_disp_ai_communication(d); return; }
    ...
    c->communication = i - 1; OLC_VAL(d) = 1; medit_disp_ai_communication(d); return; }
  case MEDIT_AI_INTELLIGENCE: {
    struct mob_ai_config *c = OLC_MOB(d)->ai_config;
    if (LOWER(*arg) == 'q') { medit_disp_ai_menu(d); return; }
  ...
  case MEDIT_AI_VOCAL_PRESENCE: if(LOWER(*arg)=='q'){medit_disp_ai_communication(d);return;}if(!*arg){write_to_output(d,"Presence: 1) None 2) Player 3) Awake player 4) Visible player 5) Any character\r\nChoice (Q cancels): ");return;}if(!medit_parse_ai_integer(arg,1,5,&i)){write_to_output(d,"Choose a presence requirement from 1 to 5: ");return;}OLC_MOB(d)->ai_config->vocal_presence=i-1;OLC_VAL(d)=1;write_to_output(d,"Presence requirement set to: %s.\r\n",medit_ai_vocal_presence_name(i-1));medit_disp_ai_communication(d);return;
  case MEDIT_AI_VOCAL_FREQUENCY: if(LOWER(*arg)=='q'){medit_disp_ai_communication(d);return;}if(!*arg){write_to_output(d,"Frequency: 1) Very Rare 2) Rare 3) Occasionally 4) Often 5) Very Often\r\nChoice (Q cancels): ");return;}if(!medit_parse_ai_integer(arg,1,5,&i)){write_to_output(d,"Choose a frequency from 1 to 5: ");return;} {static const int mins[]={120,60,30,15,5},maxs[]={300,180,90,45,15};OLC_MOB(d)->ai_config->vocal_frequency=i;OLC_MOB(d)->ai_config->vocal_cooldown_min=mins[i-1];OLC_MOB(d)->ai_config->vocal_cooldown_max=maxs[i-1];} OLC_VAL(d)=1;write_to_output(d,"Frequency set to: %s.\r\n",medit_ai_vocal_frequency_name(i));medit_disp_ai_communication(d);return;
  case MEDIT_AI_VOCAL_COOLDOWN: if(LOWER(*arg)=='q'){medit_disp_ai_communication(d);return;}if(!*arg){write_to_output(d,"Cooldown minimum and maximum (5-3600), e.g. 30 90 (Q cancels): ");return;}if(sscanf(arg,"%d %d",&i,&j)!=2){write_to_output(d,"Enter both minimum and maximum, for example: 30 90.\r\nCooldown minimum and maximum (5-3600), e.g. 30 90 (Q cancels): ");return;}if(i<5){write_to_output(d,"Minimum cooldown must be at least 5 seconds.\r\nCooldown minimum and maximum (5-3600), e.g. 30 90 (Q cancels): ");return;}if(j>3600){write_to_output(d,"Maximum cooldown cannot exceed 3600 seconds.\r\nCooldown minimum and maximum (5-3600), e.g. 30 90 (Q cancels): ");return;}if(j<i){write_to_output(d,"Maximum cooldown must be at least the minimum.\r\nCooldown minimum and maximum (5-3600), e.g. 30 90 (Q cancels): ");return;}OLC_MOB(d)->ai_config->vocal_cooldown_min=i;OLC_MOB(d)->ai_config->vocal_cooldown_max=j;OLC_VAL(d)=1;write_to_output(d,"Cooldown set to: %d-%d seconds.\r\n",i,j);medit_disp_ai_communication(d);return;
  case MEDIT_AI_VOCAL_LIMIT: if(LOWER(*arg)=='q'){medit_disp_ai_communication(d);return;}if(!*arg){write_to_output(d,"Sounds per room visit (0-10; Q cancels): ");return;}if(!medit_parse_ai_integer(arg,0,10,&i)){write_to_output(d,"Enter a per-room sound limit from 0 to 10: ");return;}OLC_MOB(d)->ai_config->vocal_room_limit=i;OLC_VAL(d)=1;write_to_output(d,"Per-room sound limit set to: %s.\r\n",i ? (i==1?"1 sound":"multiple sounds") : "Unlimited");medit_disp_ai_communication(d);return;
  case MEDIT_AI_VOCALIZATIONS: {
    struct mob_ai_config *c=OLC_MOB(d)->ai_config; char cmd=LOWER(*arg); int line=atoi(arg+1)-1;
    if(cmd=='q'){medit_disp_ai_communication(d);return;} if(cmd=='h'){medit_disp_ai_help(d,MEDIT_AI_VOCALIZATIONS,"Creature Sounds","Creature sounds are room messages, not spoken dialogue.","Enter a displayed sound number to edit it, or A to add one.","Short sounds and full room descriptions are both valid.");return;} if(cmd=='p'){snprintf(arg,MAX_INPUT_LENGTH,"9");OLC_MODE(d)=MEDIT_AI_COMMUNICATION;medit_parse(d,arg);return;} if(cmd=='a'){if(c->vocalization_count>=AI_VOCALIZATION_MAX_LINES){write_to_output(d,"Maximum of %d sounds reached.\r\n",AI_VOCALIZATION_MAX_LINES);medit_disp_ai_vocalizations(d);return;}OLC_MODE(d)=MEDIT_AI_VOCALIZATION_ADD;medit_ai_vocal_prompt(d);return;} if(isdigit((unsigned char)*arg)&&!arg[1]){line=atoi(arg)-1;if(line>=0&&line<c->vocalization_count){OLC_VAL(d)=line;medit_disp_ai_vocalization_entry(d);return;}} medit_disp_ai_vocalizations(d);return;
  }
  case MEDIT_AI_VOCALIZATION_ADD: {const char *error;if(LOWER(*arg)=='q'){medit_disp_ai_vocalizations(d);return;}if((error=medit_validate_creature_sound(arg))!=NULL){write_to_output(d,"%s\r\n",error);medit_ai_vocal_prompt(d);return;}if(!mob_ai_vocalization_set(OLC_MOB(d)->ai_config,OLC_MOB(d)->ai_config->vocalization_count,arg)){write_to_output(d,"Creature sound could not be saved.\r\n");medit_ai_vocal_prompt(d);return;}OLC_VAL(d)=1;write_to_output(d,"Creature sound added.\r\n");medit_disp_ai_vocalizations(d);return;}
  case MEDIT_AI_VOCALIZATION_EDIT: {const char *error;if(LOWER(*arg)=='q'){medit_disp_ai_vocalization_entry(d);return;}if((error=medit_validate_creature_sound(arg))!=NULL){write_to_output(d,"%s\r\n",error);medit_ai_vocal_prompt(d);return;}if(!mob_ai_vocalization_set(OLC_MOB(d)->ai_config,OLC_VAL(d),arg)){write_to_output(d,"Creature sound could not be saved.\r\n");medit_ai_vocal_prompt(d);return;}OLC_VAL(d)=1;write_to_output(d,"Creature sound updated.\r\n");medit_disp_ai_vocalizations(d);return;}
  case MEDIT_AI_VOCALIZATION_INDEX: {struct mob_ai_config*c=OLC_MOB(d)->ai_config;int n=OLC_VAL(d);char cmd=LOWER(*arg);if(n<0||n>=c->vocalization_count){medit_disp_ai_vocalizations(d);return;}if(cmd=='q'){medit_disp_ai_vocalizations(d);return;}if(cmd=='1'){OLC_MODE(d)=MEDIT_AI_VOCALIZATION_EDIT;medit_ai_vocal_prompt(d);return;}if(cmd=='2'){c->vocal_disabled_mask^=1U<<n;OLC_VAL(d)=1;medit_disp_ai_vocalization_entry(d);return;}if(cmd=='3'){int new_index=c->vocalization_count;if(new_index<AI_VOCALIZATION_MAX_LINES&&mob_ai_vocalization_set(c,new_index,c->vocalization[n])){if(c->vocal_disabled_mask&(1U<<n))c->vocal_disabled_mask|=1U<<new_index;else c->vocal_disabled_mask&=~(1U<<new_index);OLC_VAL(d)=1;}medit_disp_ai_vocalizations(d);return;}if(cmd=='4'&&n>0){mob_ai_vocalization_move(c,n,n-1);OLC_VAL(d)=1;medit_disp_ai_vocalizations(d);return;}if(cmd=='5'&&n+1<c->vocalization_count){mob_ai_vocalization_move(c,n,n+1);OLC_VAL(d)=1;medit_disp_ai_vocalizations(d);return;}if(cmd=='d'){mob_ai_vocalization_delete(c,n);OLC_VAL(d)=1;medit_disp_ai_vocalizations(d);return;}medit_disp_ai_vocalization_entry(d);return;}
  ...
  case MEDIT_AI_DIALOGUE:
    if (LOWER(*arg)=='q') { medit_disp_ai_communication(d); return; }
  ...
  case MEDIT_AI_ROUTINE_MODE: i=atoi(arg); if(i<1||i>4){write_to_output(d,"Choose 1-4.\r\n");}else{OLC_MOB(d)->ai_config->routine_mode=i-1;OLC_MOB(d)->ai_config->schedule_enabled=(i>=3);OLC_VAL(d)=1;}medit_disp_ai_schedule(d);return;
  case MEDIT_AI_ROUTINE_RANDOM: {struct mob_ai_config*c=OLC_MOB(d)->ai_config;char cmd=LOWER(*arg);int n;if(cmd=='q'){medit_disp_ai_schedule(d);return;}if(medit_parse_ai_integer(arg,1,c->random_destination_count,&n)){OLC_VAL(d)=n-1;medit_disp_ai_routine_random_entry(d);return;}if(cmd=='a'){if(c->random_destination_count>=AI_ROUTINE_DEST_MAX)write_to_output(d,"The random destination list is full.\r\n");else{OLC_VAL(d)=c->random_destination_count;OLC_MODE(d)=MEDIT_AI_ROUTINE_RANDOM_VNUM;write_to_output(d,"\r\nAdd Random Destination\r\n----------------------\r\n\r\nEnter the destination room VNUM.\r\n\r\nThe NPC will calculate its own route from its current live room.\r\n\r\nRoom VNUM (Q cancels): ");return;}}else if(cmd=='w'){OLC_MODE(d)=MEDIT_AI_ROUTINE_WAIT;write_to_output(d,"Wait minimum and maximum seconds: ");return;}medit_disp_ai_routine_random(d);return;}
  case MEDIT_AI_ROUTINE_RANDOM_ENTRY: {struct mob_ai_config*c=OLC_MOB(d)->ai_config;int n=OLC_VAL(d);char cmd=LOWER(*arg);if(cmd=='q'){medit_disp_ai_routine_random(d);return;}if(n<0||n>=c->random_destination_count){medit_disp_ai_routine_random(d);return;}if(cmd=='1'){OLC_MODE(d)=MEDIT_AI_ROUTINE_RANDOM_VNUM;write_to_output(d,"Room VNUM (Q cancels): ");return;}if(cmd=='2'){c->random_destinations[n].enabled=!c->random_destinations[n].enabled;OLC_VAL(d)=1;}else if(cmd=='d'){memmove(&c->random_destinations[n],&c->random_destinations[n+1],sizeof(c->random_destinations[0])*(c->random_destination_count-n-1));c->random_destination_count--;OLC_VAL(d)=1;medit_disp_ai_routine_random(d);return;}medit_disp_ai_routine_random_entry(d);return;}
  case MEDIT_AI_ROUTINE_RANDOM_VNUM: {struct mob_ai_config*c=OLC_MOB(d)->ai_config;int n=OLC_VAL(d),v;const char *error;if(LOWER(*arg)=='q'){write_to_output(d,"Destination creation cancelled.\r\n");medit_disp_ai_routine_random(d);return;}if(!medit_parse_ai_integer(arg,1,INT_MAX,&v)) error="Enter a numeric room VNUM, or Q to cancel.";else error=medit_validate_routine_room(d,v,n<c->random_destination_count?n:-1,NULL);if(error){write_to_output(d,"%s\r\n",error);medit_disp_ai_routine_random(d);return;}if(n==c->random_destination_count){c->random_destinations[n].room_vnum=v;c->random_destinations[n].enabled=TRUE;c->random_destinations[n].weight=1;c->random_destination_count++;}else if(n>=0&&n<c->random_destination_count)c->random_destinations[n].room_vnum=v;else{medit_disp_ai_routine_random(d);return;}OLC_VAL(d)=1;write_to_output(d,"Random destination added:\r\n\r\n    %d — %s\r\n",v,world[real_room(v)].name);medit_disp_ai_routine_random(d);return;}
  case MEDIT_AI_ROUTINE_WAIT: {int lo,hi;if(sscanf(arg,"%d %d",&lo,&hi)!=2||lo<1||hi<lo||hi>3600)write_to_output(d,"Enter a valid minimum and maximum.\r\n");else{OLC_MOB(d)->ai_config->random_wait_min=lo;OLC_MOB(d)->ai_config->random_wait_max=hi;OLC_VAL(d)=1;}medit_disp_ai_routine_random(d);return;}
  case MEDIT_AI_SCHEDULE: { struct mob_ai_config*c=OLC_MOB(d)->ai_config; char cmd=LOWER(*arg), report[MAX_STRING_LENGTH]; if(cmd=='q'){medit_disp_ai_menu(d);return;} if(cmd=='h'){medit_disp_ai_help(d, MEDIT_AI_SCHEDULE, "Schedule", "Schedules describe routines; normal MEDIT flags remain the source of ordinary movement restrictions.", "Preview and validation do not modify the NPC. Preview: Show the day's effective activity timeline.", "Daily schedule and patrol route editors retain detailed activity settings.");return;} if(cmd=='1'){OLC_MODE(d)=MEDIT_AI_ROUTINE_MODE;write_to_output(d,"Routine Mode\r\n1) No Routine\r\n2) Random Destination Travel\r\n3) Scheduled Destinations\r\n4) Random + Scheduled\r\nChoice: ");return;} else if(cmd=='2'){medit_disp_ai_routine_random(d);return;} else if(cmd=='3'){medit_disp_ai_schedule_entries(d);return;} else if(cmd=='4'){write_to_output(d,"Travel Settings\r\n---------------\r\nBoundary: %s\r\nFailure Policy: %d\r\nMovement Cadence: %d seconds\r\nMaximum Search: 128 rooms\r\n\r\nQ) Return\r\n",c->routine_boundary==AI_ROUTINE_CURRENT_ZONE?"Current Zone":"Allowed Zones",c->routine_failure,c->movement_delay);} else if(cmd=='5'){ai_actor_schedule_preview(c,((35*time_info.month)+(time_info.day+1))%7,time_info.hours,report,sizeof(report));write_to_output(d,"Daily Routine Preview\r\n%s",report);} else if(cmd=='6'){ai_actor_schedule_validate(c,report,sizeof(report));write_to_output(d,"Validation\r\n%s",report);} else if(cmd=='a'){medit_disp_ai_patrol_routes(d);return;} else write_to_output(d,"Invalid Daily Routine choice.\r\n");medit_disp_ai_schedule(d);return; }

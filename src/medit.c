...
static void medit_disp_ai_targets(struct descriptor_data *d) { struct mob_ai_config*c=OLC_MOB(d)->ai_config; int i; write_to_output(d,"\r\nTarget Preferences (signed -100..100)\r\n"); for(i=0;i<AI_TARGET_WEIGHTS;i++) write_to_output(d,"%d) Weight: %d\r\n",i+1,c->target_weight[i]); write_to_output(d,"Q) Return\r\nChoice: "); OLC_MODE(d)=MEDIT_AI_TARGETS; }

...
void medit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case MEDIT_MAIN_MENU:
    ...
    case 'H':
      ...
      write_to_output(d, "Body profile (-1 = Automatic):\r\n");
...
/* Numerical responses. */
  ...
  case MEDIT_BODY_PROFILE:
    if (!is_number(arg) || i < -1 || i >= NUM_BODY_PROFILES) {
      write_to_output(d, "Choose -1 for Automatic or a profile number from 0 to %d: ", NUM_BODY_PROFILES - 1);
      return;
    }
    GET_MOB_BODY_PROFILE(OLC_MOB(d)) = i < 0 ? BODY_PROFILE_NONE : i;
    MOB_BODY_PROFILE_SET(OLC_MOB(d)) = (i >= 0);
    OLC_VAL(d) = TRUE;
    medit_disp_menu(d);
    return;

...
void medit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case MEDIT_AI_MEMORY: if(LOWER(*arg)=='q'){medit_disp_ai_menu(d);return;} if(LOWER(*arg)=='h'){write_to_output(d,"Memory is per live NPC instance, shared across actor categories, and is discarded at reboot. Expired records are removed before a new record is kept.\r\n");medit_disp_ai_memory(d);return;} if(!medit_parse_ai_integer(arg,1,22,&i)){medit_disp_ai_memory(d);return;} OLC_VAL(d)=i;OLC_MODE(d)=MEDIT_AI_MEMORY_VALUE;write_to_output(d,"Memory field %d\r\nCurrent value: %d\r\nUse Y/N/T for a toggle, or enter the documented whole-number range. H) Help  Q) Cancel\r\nChoice: ",i,(i<=13?(&OLC_MOB(d)->ai_config->memory_enabled)[i-1]:(&OLC_MOB(d)->ai_config->remember_attacks)[i-14]));return;
  ...
  case MEDIT_AI_SCHEDULE: { struct mob_ai_config*c=OLC_MOB(d)->ai_config; char cmd=LOWER(*arg), report[MAX_STRING_LENGTH]; if(cmd=='q'){medit_disp_ai_menu(d);return;} if(cmd=='h'){write_to_output(d,"Schedules use game hours. Travel uses engine movement/path checks; validation is non-mutating and reports invalid rooms/routes. Patrol routes are room-VNUM waypoints, with loop, ping-pong, or once modes.\r\n");medit_disp_ai_schedule(d);return;} if(cmd=='1'){c->schedule_enabled=!c->schedule_enabled;OLC_VAL(d)=1;} else if(cmd=='2'){c->resume_after_interrupt=!c->resume_after_interrupt;OLC_VAL(d)=1;} else if(cmd=='3'){if(OLC_STORAGE(d)){free(OLC_STORAGE(d));OLC_STORAGE(d)=NULL;}OLC_MODE(d)=MEDIT_AI_SCHEDULE_FAILURE;write_to_output(d,"Failure policy (0-%d): ",AI_FAILURE_MAX-1);return;} else if(cmd>='4'&&cmd<='8'){OLC_VAL(d)=cmd-'4';OLC_MODE(d)=MEDIT_AI_SCHEDULE_ROOM;write_to_output(d,"Room VNUM (0 or none clears): ");return;} else if(cmd=='a'){medit_disp_ai_schedule_entries(d);return;} else if(cmd=='b'){medit_disp_ai_patrol_routes(d);return;} else if(cmd=='c'){ai_actor_schedule_preview(c,((35*time_info.month)+(time_info.day+1))%7,time_info.hours,report,sizeof(report));write_to_output(d,"%s",report);} else if(cmd=='d'){ai_actor_schedule_validate(c,report,sizeof(report));write_to_output(d,"%s",report);} else write_to_output(d,"Invalid schedule choice.\r\n");medit_disp_ai_schedule(d);return; }

...
static void medit_disp_ai_menu(struct descriptor_data *d)
{
  ...
  write_to_output(d, "\r\n%s                    AI Actor Configuration%s\r\n\r\n%sProfile%s\r\n  %s1)%s Profile Mode: %s%s%s\r\n  %s2)%s Role: %s%s%s\r\n  %s3)%s Movement: %s%s%s\r\n  %s4)%s Personality : %sStored traits — press 4 for details%s\r\n\r\n%sInteraction%s\r\n  %s5)%s Social Behavior: %s%s%s; greeting %s; ambient %s/%s\r\n  %s6)%s Dialogue Lines: %s%d authored lines across %d event pools%s\r\n  %s7)%s Perception  : %sStandard room-event awareness%s\r\n  %s8)%s Memory      : %s%s — press 8 for details%s\r\n  %s9)%s Threat Response: %sDefault escalation behavior%s\r\n  %s0)%s Combat Reactions: %sRuntime reactions and targeting%s\r\n\r\n%sWorld Behavior%s\r\n  %sS)%s Schedules and Patrol Routines: %s%s; %d entries; %d patrol routes%s\r\n\r\n%sTools%s\r\n  %sP)%s Preview Compiled Profile: Show effective compiled runtime profile\r\n  %sV)%s Validate    : Check configuration without changing it\r\n  %sR)%s Reset to Inferred Defaults: Rebuild from inferred defaults\r\n  %sH)%s Help   %sQ)%s Return\r\nChoice: %s", title,normal,title,normal,key,normal,value,c->mode == MOB_AI_CUSTOM ? "Custom" : c->mode == MOB_AI_INFERRED_OVERRIDES ? "Overrides" : "Inferred",normal,key,normal,value,ai_actor_config_role_name(c->role),normal,key,normal,value,ai_actor_config_movement_name(c->movement),normal,key,normal,value,normal,title,normal,key,normal,value,ai_social_style_name(c->social),normal,c->greeting_enabled ? "on" : "off",c->ambient_speech_enabled ? "on" : "off",c->ambient_emotes_enabled ? "on" : "off",key,normal,value,lines,AI_DIALOGUE_CATEGORIES,normal,key,normal,value,normal,key,normal,value,medit_ai_state(d,c->memory_enabled),normal,key,normal,value,normal,key,normal,value,normal,title,normal,key,normal,value,c->schedule_enabled ? "Enabled" : "Disabled",c->schedule_count,c->patrol_count,normal,title,normal,key,normal,key,normal,key,normal,key,normal,key,normal,normal);
...
static int medit_is_ai_mode(int mode)
{
  return mode >= MEDIT_AI_MENU && mode <= MEDIT_AI_SCHEDULE_INDEX;
}
...
void medit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case MEDIT_AI_MENU:
    switch (LOWER(*arg)) {
      ...
      case 'h': write_to_output(d, "Configures Profile Mode, Role:, Movement:, Personality, Social Behavior, Dialogue Lines, Perception, Memory, Threat Response, Combat Reactions, Schedules and Patrol Routines, Preview Compiled Profile, and Reset to Inferred Defaults.\r\n"); medit_disp_ai_menu(d); return;

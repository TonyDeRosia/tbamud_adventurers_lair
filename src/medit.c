...
static void medit_disp_ai_schedule(struct descriptor_data *d) { struct mob_ai_config*c=OLC_MOB(d)->ai_config; const char *m[]={"No Routine","Random Destination Travel","Scheduled Destinations","Random + Scheduled","Advanced Patrol"}; write_to_output(d,"\r\nDaily Routine\r\n-------------\r\n\r\nMode:\r\n    %s\r\n\r\nRandom Destinations:\r\n    %d\r\n\r\nScheduled Destinations:\r\n    %d\r\n\r\nTravel Boundary:\r\n    %s\r\n\r\nWait After Random Arrival:\r\n    %d-%d seconds\r\n\r\n1) Change Routine Mode\r\n2) Random Destinations\r\n3) Scheduled Destinations\r\n4) Travel Settings\r\n5) Preview Routine\r\n6) Validate Routine\r\n\r\nA) Advanced Patrol Editor\r\nH) Help\r\nQ) Return\r\nChoice: ",m[MAX(0,MIN(c->routine_mode,4))],c->random_destination_count,c->schedule_count,c->routine_boundary==AI_ROUTINE_CURRENT_ZONE?"Current Zone":"Allowed Zones",c->random_wait_min,c->random_wait_max); OLC_MODE(d)=MEDIT_AI_SCHEDULE; }
...
void medit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case MEDIT_AI_ROUTINE_RANDOM: {struct mob_ai_config*c=OLC_MOB(d)->ai_config;char cmd=LOWER(*arg);int n=atoi(arg)-1;if(cmd=='q'){medit_disp_ai_schedule(d);return;}if(isdigit((unsigned char)*arg)&&!arg[1]&&n>=0&&n<c->random_destination_count){OLC_VAL(d)=n;medit_disp_ai_routine_random_entry(d);return;}if(cmd=='a'){if(c->random_destination_count>=AI_ROUTINE_DEST_MAX)write_to_output(d,"Maximum destinations reached.\r\n");else{c->random_destinations[c->random_destination_count].enabled=TRUE;c->random_destinations[c->random_destination_count].weight=1;OLC_VAL(d)=c->random_destination_count++;OLC_MODE(d)=MEDIT_AI_ROUTINE_RANDOM_VNUM;write_to_output(d,"Room VNUM: ");return;}}else if(cmd=='w'){OLC_MODE(d)=MEDIT_AI_ROUTINE_WAIT;write_to_output(d,"Wait minimum and maximum seconds: ");return;}medit_disp_ai_routine_random(d);return;}
  case MEDIT_AI_ROUTINE_RANDOM_ENTRY: {struct mob_ai_config*c=OLC_MOB(d)->ai_config;int n=OLC_VAL(d);char cmd=LOWER(*arg);if(cmd=='q'){medit_disp_ai_routine_random(d);return;}if(n<0||n>=c->random_destination_count){medit_disp_ai_routine_random(d);return;}if(cmd=='1'){OLC_MODE(d)=MEDIT_AI_ROUTINE_RANDOM_VNUM;write_to_output(d,"Room VNUM: ");return;}if(cmd=='2'){c->random_destinations[n].enabled=!c->random_destinations[n].enabled;OLC_VAL(d)=1;}else if(cmd=='d'){memmove(&c->random_destinations[n],&c->random_destinations[n+1],sizeof(c->random_destinations[0])*(c->random_destination_count-n-1));c->random_destination_count--;OLC_VAL(d)=1;medit_disp_ai_routine_random(d);return;}medit_disp_ai_routine_random_entry(d);return;}
  case MEDIT_AI_ROUTINE_RANDOM_VNUM: {struct mob_ai_config*c=OLC_MOB(d)->ai_config;int n=OLC_VAL(d),v=atoi(arg);if(n<0||n>=c->random_destination_count||v<=0||real_room(v)==NOWHERE){write_to_output(d,"Invalid room VNUM; destination was not changed.\r\n");medit_disp_ai_routine_random(d);return;}c->random_destinations[n].room_vnum=v;OLC_VAL(d)=1;medit_disp_ai_routine_random_entry(d);return;}

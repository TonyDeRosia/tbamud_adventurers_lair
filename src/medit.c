...
static int medit_is_ai_mode(int mode)
{
  return mode >= MEDIT_AI_MENU && mode <= MEDIT_AI_PATROL_WAYPOINT_DELETE;
}
...
void medit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case MEDIT_AI_PERCEPTION:
    if (LOWER(*arg)=='q') { medit_disp_ai_menu(d); return; }
    if (LOWER(*arg)=='h') { write_to_output(d, "Perception toggles decide which room events are noticed. Hearing, observation, suspicion, and recognition are 0-100 runtime sensitivities.\r\n"); medit_disp_ai_perception(d); return; }
  ...
  case MEDIT_AI_PERCEPTION_VALUE: { struct mob_ai_config*c=OLC_MOB(d)->ai_config; int *v=&c->notice_entry+OLC_VAL(d)-1;
    ...
    if (OLC_VAL(d)<=12) { if (LOWER(*arg)=='t') i=!*v; else if (!medit_parse_ai_boolean(arg,&i)) { write_to_output(d,"Please answer Y or N: "); return; } }
  ...
  case MEDIT_AI_SCHEDULE_ENTRIES: {struct mob_ai_config*c=OLC_MOB(d)->ai_config;char cmd=LOWER(*arg);int n=atoi(arg+1)-1;if(cmd=='q'){medit_disp_ai_schedule(d);return;}if(cmd=='a'){struct ai_schedule_entry z;memset(&z,0,sizeof(z));z.enabled=TRUE;z.day_mask=AI_DAY_MASK_ALL;z.activity=AI_SCHEDULE_REMAIN;z.destination=AI_DEST_CURRENT_ROOM;z.max_attempts=3;if(ai_schedule_add(c,&z))OLC_VAL(d)=1;else write_to_output(d,"Maximum entry count reached.\r\n");}else if(cmd=='e'){if(n<0||n>=c->schedule_count)write_to_output(d,"Invalid entry index.\r\n");else{medit_schedule_store(d,-1,c->schedules[n].id,-1);medit_disp_ai_schedule_entry(d);return;}}else if(cmd=='d'){if(n<0||n>=c->schedule_count)write_to_output(d,"Invalid entry index.\r\n");else{medit_schedule_store(d,-1,c->schedules[n].id,-1);OLC_MODE(d)=MEDIT_AI_SCHEDULE_DELETE;write_to_output(d,"Delete entry? (Y/N): ");return;}}else if(cmd=='u'||cmd=='n'||cmd=='c'||cmd=='t'){if(n<0||n>=c->schedule_count)write_to_output(d,"Invalid entry index.\r\n");else if(cmd=='u'&&ai_schedule_move(c,n,n-1))OLC_VAL(d)=1;else if(cmd=='n'&&ai_schedule_move(c,n,n+1))OLC_VAL(d)=1;else if(cmd=='c'){if(ai_schedule_duplicate(c,n))OLC_VAL(d)=1;else write_to_output(d,"Maximum entry count reached.\r\n");}else if(cmd=='t'){c->schedules[n].enabled=!c->schedules[n].enabled;OLC_VAL(d)=1;}else write_to_output(d,"Invalid entry index.\r\n");}else write_to_output(d,"Invalid entry command.\r\n");medit_disp_ai_schedule_entries(d);return;}

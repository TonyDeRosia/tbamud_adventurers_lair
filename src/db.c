...
static void interpret_espec(const char *keyword, const char *value, int i, int nr)
{
  ...
  if (value && !str_cmp(keyword, "AIConfigThreat")) { struct mob_ai_config *c=mob_proto[i].ai_config; if(!c || sscanf(value,"%d %d %d %d %d %d %d %d %d %d %d %d",&c->threat_enabled[0],&c->threat_enabled[1],&c->threat_enabled[2],&c->threat_enabled[3],&c->threat_enabled[4],&c->threat_enabled[5],&c->threat_enabled[6],&c->threat_enabled[7],&c->threat_enabled[8],&c->threat_enabled[9],&c->threat_cooldown,&c->repeated_event_window)!=12) log("SYSERR: Bad AIConfigThreat format in mob #%d",nr); else mob_ai_config_validate(c); return; }

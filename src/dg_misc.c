                 ...
                 int type, char *cmd)
{
  ...
  enum combat_skill_result result = COMBAT_SKILL_NOT_ATTEMPTED;
  int proficiency;
  ...
  if (!vict || vict == mob || IN_ROOM(mob) == NOWHERE || IN_ROOM(vict) != IN_ROOM(mob)) {
    script_log("Trigger: %s, VNum %d. dg_skill target is absent or not in the mob's room.",
      GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig));
    return;
  }
  if (GET_WAIT_STATE(mob) > 0)
    return;
  proficiency = MIN(90, 30 + GET_LEVEL(mob));
  if (!str_cmp(skill, "backstab"))
    result = perform_backstab(mob, vict, proficiency, FALSE);
  else if (!str_cmp(skill, "bash")) {
    result = perform_bash(mob, vict, proficiency, FALSE, TRUE);
  } else if (!str_cmp(skill, "kick")) {
    result = perform_kick(mob, vict, proficiency, FALSE, TRUE);
  }
  if (result == COMBAT_SKILL_ATTEMPTED)

...
static void get_check_money(struct char_data *ch, struct obj_data *obj)
{
  ...
  extract_obj(obj);
  increase_money_gold(ch, (long long)value);
  if (value == 1)
...
static void perform_drop_currency(struct char_data *ch, int amount, int denom, int subcmd)
{
  ...
  {
    struct obj_data *obj = create_money((int)amount_gold, denom);
    if (!obj) {
...
ACMD(do_drop)
{
  ...
  if (amount && (subcmd == SCMD_JUNK)) {
    ...
    increase_money_gold(ch, (long long)amount);
  }
}
...
static void perform_give_currency(struct char_data *ch, struct char_data *vict, int amount, int denom)
{
  ...
  long long gold = amount;
  if (gold <= 0) {
    send_to_char(ch, "Give how much of what?\r\n");
    return;
  }

  if ((GET_MONEY(ch) < gold) && (IS_NPC(ch) || (GET_LEVEL(ch) < LVL_GOD))) {
    send_to_char(ch, "You don't have that much currency!\r\n");
    return;
  }
...
static int weapon_wear_position(struct char_data *ch, const struct obj_data *obj)
{
  if (GET_EQ(ch, WEAR_WIELD) && is_offhand_weapon(obj) && !is_two_hander(obj))
    return WEAR_HOLD;
  return WEAR_WIELD;
}
...
#define WEAPON_REJECT(message) do { if (show_message) send_to_char(ch, message); return FALSE; } while (0)
  ...
  if (is_two_hander(obj) && is_offhand_weapon(obj))
    WEAPON_REJECT("That weapon has incompatible two-handed and offhand properties.\r\n");
  ...
  if (where == WEAR_WIELD) {
    ...
    if (!IS_NPC(ch)) {
      ...
      if (GET_LEVEL(ch) < dual_level)
        WEAPON_REJECT("You have not reached the level required to learn Dual Wield.\r\n");
      if (GET_SKILL(ch, SKILL_DUAL_WIELD) <= 0)
        WEAPON_REJECT("Dual Wield is available to you, but you have not learned it; your current skill is insufficient.\r\n");
    }
...
static int perform_wear(struct char_data *ch, struct obj_data *obj, int where)
{
  ...
  /* dual wield + two hander rules begin */
  {
    ...
    /* A shield conflicts with a held weapon, not a utility Hold item. */
    if (prim && GET_OBJ_TYPE(prim) == ITEM_WEAPON) {
      if (sh && held && GET_OBJ_TYPE(held) == ITEM_WEAPON) {
    ...
    if (where == WEAR_SHIELD) {
      if (held && GET_OBJ_TYPE(held) == ITEM_WEAPON) {
        send_to_char(ch, "You cannot use a shield while holding a weapon.\r\n");
        return FALSE;
      }
      if (character_is_using_two_hander(ch)) {
    ...
    if (where == WEAR_HOLD) {
      ...
      if (!obj || GET_OBJ_TYPE(obj) != ITEM_WEAPON) {
        /* Utility Hold items coexist with shields, but not two-handers. */
        if (character_is_using_two_hander(ch)) {
          send_to_char(ch, "Your weapon requires both hands.\r\n");
          return FALSE;
        }
  ...
  if (!CAN_WEAR(obj, wear_bitvectors[where]) &&
      !(where == WEAR_HOLD && GET_OBJ_TYPE(obj) == ITEM_WEAPON && OBJ_FLAGGED(obj, ITEM_OFFHAND)) &&
      !(where == WEAR_WIELD && GET_OBJ_TYPE(obj) == ITEM_WAND)) {
...
int find_eq_pos(struct char_data *ch, struct obj_data *obj, char *arg)
{
  ...
  if (!arg || !*arg) {
        /* Adventurer's Lair: LIGHT and HOLD are independent equipment slots.
     * A real light source always resolves to WEAR_LIGHT.  A non-weapon
     * object explicitly flagged HOLD resolves to WEAR_HOLD. */
    if (GET_OBJ_TYPE(obj) == ITEM_LIGHT)
      return WEAR_LIGHT;

    if (GET_OBJ_TYPE(obj) != ITEM_WEAPON && CAN_WEAR(obj, ITEM_WEAR_HOLD))
      return WEAR_HOLD;
...
ACMD(do_wear)
{
  ...
  if (dotmode == FIND_ALL) {
    /* Equip ordinary wearables and prefer a normal or two-handed primary.
     * Offhand-capable weapons are deferred so a normal primary wins regardless
     * of inventory order. */
    for (obj = ch->carrying; obj; obj = next_obj) {
      next_obj = obj->next_content;
      if (!CAN_SEE_OBJ(ch, obj))
    ...
    if (!character_is_using_two_hander(ch)) {
      ...
      for (obj = ch->carrying; obj; obj = next_obj) {
        ...
        if (!GET_EQ(ch, WEAR_WIELD)) {
          if (perform_wear(ch, obj, WEAR_WIELD))
            items_worn++;
          continue;
        }
...
ACMD(do_grab)
{
  ...
  if (!*arg)
    ...
    send_to_char(ch, "Use LIGHT to equip a light source.\r\n");
...
ACMD(do_light)
{
  ...
  if (!*arg)
    ...
    send_to_char(ch, "That is not a light source.\r\n");
  else
...
static int auction_item_level(struct obj_data *obj)
{
  if (!obj)
    return 0;
  return GET_OBJ_LEVEL(obj);
}

static const char *auction_currency_name(void)
{
  return "gold";
}
...
{
  snprintf(buf, bufsz,
           "%sMarket:%s %s%s%s is selling %s (Level %s%d%s, Num %s%d%s). Auction is for %s%s%s.",
           KGRN, KNRM,
...
{
  snprintf(buf, bufsz,
           "%sAuction:%s %s%s%s is auctioning %s (Level %s%d%s, Num %s%d%s). Current bid is %s%lld%s.",
           KMAG, KNRM,
           KYEL, seller ? GET_NAME(seller) : "Someone", KNRM,
           auction_item_display_name(obj),
           KCYN, auction_item_level(obj), KNRM,
...
void auction_update(void)
{
  ...
  if (live_auction.going == 2) {
    snprintf(msg, sizeof(msg), "%sAuction:%s %s going once for %s%lld%s %s.",

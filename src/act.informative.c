      ...
      if (short_list) {
        ...
        num_can_see++;
        {
          ...
          if (GET_CLAN_ID(tch) > 0) {
            ...
            size_t clan_name_len = strlen(clan_name);

            if (clan_name_len >= 2 && clan_name[clan_name_len - 2] == '\t' &&
                clan_name[clan_name_len - 1] == 'n')
              clan_name_len -= 2;

            snprintf(clancol, sizeof(clancol), "[%.*s]\\tn", (int)clan_name_len, clan_name);
          }

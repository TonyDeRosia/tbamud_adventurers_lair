...
int write_mobile_espec(mob_vnum mvnum, struct char_data *mob, FILE *fd)
{
  ...
  if (mob->ai_config) {
    ...
    for(i=0;i<c->schedule_count;i++){struct ai_schedule_entry *e=&c->schedules[i];fprintf(fd,"AISchedule: %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",e->id,e->enabled,e->start_hour,e->end_hour,e->day_mask,e->priority,e->activity,e->destination,e->destination_value,e->arrival_action,e->departure_action,e->interruption_policy,e->failure_policy,e->max_travel_time,e->max_attempts,e->wait_duration,e->route_id);}

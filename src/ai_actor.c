...
struct mob_ai_config *mob_ai_config_new(void)
{
  ...
  c->routine_mode=AI_ROUTINE_NONE; c->routine_boundary=AI_ROUTINE_CURRENT_ZONE;
  c->routine_failure=AI_ROUTINE_RETRY; c->random_wait_min=30; c->random_wait_max=90;
  c->vocal_presence=AI_VOCAL_PRESENCE_PLAYER; c->vocal_frequency=3; c->vocal_cooldown_min=30; c->vocal_cooldown_max=90; c->vocal_room_limit=2;
...
struct mob_ai_config *mob_ai_config_copy(const struct mob_ai_config *from) {
  ...
  *c = *from;
  ...
  for (i = 0; i < AI_VOCALIZATION_MAX_LINES; i++) c->vocalization[i] = from->vocalization[i] ? strdup(from->vocalization[i]) : NULL;
  return c;
}
void mob_ai_config_free(struct mob_ai_config *c) { int k,i; if (!c) return; for(k=0;k<AI_DIALOGUE_CATEGORIES;k++) for(i=0;i<AI_DIALOGUE_MAX_LINES;i++) free(c->dialogue[k][i]); for(i=0;i<AI_VOCALIZATION_MAX_LINES;i++) free(c->vocalization[i]); free(c); }
...
int mob_ai_vocalization_set(struct mob_ai_config *c, int i, const char *line)
{ char clean[AI_VOCALIZATION_LINE_MAX], *p, *end; if(!c||i<0||i>=AI_VOCALIZATION_MAX_LINES||!line||strlen(line)>=sizeof(clean))return FALSE; for(p=(char *)line;*p;p++) if(((unsigned char)*p<32)||((unsigned char)*p==127))return FALSE; strlcpy(clean,line,sizeof(clean)); p=clean; skip_spaces(&p); end=p+strlen(p); while(end>p&&isspace((unsigned char)end[-1]))*--end='\0'; if(!*p)return FALSE; free(c->vocalization[i]); c->vocalization[i]=strdup(p); if(i>=c->vocalization_count)c->vocalization_count=i+1; return c->vocalization[i]!=NULL; }
int mob_ai_vocalization_delete(struct mob_ai_config *c, int i)
{ int n; unsigned int low; if(!c||i<0||i>=c->vocalization_count)return FALSE; free(c->vocalization[i]); for(n=i;n+1<c->vocalization_count;n++)c->vocalization[n]=c->vocalization[n+1]; c->vocalization[--c->vocalization_count]=NULL; low=c->vocal_disabled_mask&((1U<<i)-1U); c->vocal_disabled_mask=low|((c->vocal_disabled_mask>>(i+1))<<i); return TRUE; }
int mob_ai_vocalization_move(struct mob_ai_config *c, int from, int to)
{ char *line; int n, disabled; if(!c||from<0||to<0||from>=c->vocalization_count||to>=c->vocalization_count)return FALSE; line=c->vocalization[from]; disabled=!!(c->vocal_disabled_mask&(1U<<from)); if(from<to)for(n=from;n<to;n++){c->vocalization[n]=c->vocalization[n+1]; if(c->vocal_disabled_mask&(1U<<(n+1)))c->vocal_disabled_mask|=1U<<n;else c->vocal_disabled_mask&=~(1U<<n);}else for(n=from;n>to;n--){c->vocalization[n]=c->vocalization[n-1]; if(c->vocal_disabled_mask&(1U<<(n-1)))c->vocal_disabled_mask|=1U<<n;else c->vocal_disabled_mask&=~(1U<<n);}c->vocalization[to]=line; if(disabled)c->vocal_disabled_mask|=1U<<to;else c->vocal_disabled_mask&=~(1U<<to);return TRUE; }
...
{ const char *line; if (!mob || !mob->ai_prof || mob->ai_prof->communication != AI_COMM_SPEAK) return; line=ai_pick_dialogue(mob,category,FALSE); if (!line) line=fallback; if (!line) return; do_say(mob,(char *)line,0,0); mob->ai_state->last_spoke=now; mob->ai_state->last_room_spoke=now; mob->ai_state->last_room_vnum_spoke=world[IN_ROOM(mob)].number; }
...
static int ai_actor_vocal_presence_ok(struct char_data *mob)
{ struct char_data *p; int policy=mob->ai_config->vocal_presence; if(policy==AI_VOCAL_PRESENCE_NONE)return TRUE; for(p=world[IN_ROOM(mob)].people;p;p=p->next_in_room){if(p==mob)continue; if(policy==AI_VOCAL_PRESENCE_CHARACTER)return TRUE; if(IS_NPC(p))continue; if(policy==AI_VOCAL_PRESENCE_PLAYER)return TRUE; if(policy==AI_VOCAL_PRESENCE_AWAKE_PLAYER&&AWAKE(p))return TRUE; if(policy==AI_VOCAL_PRESENCE_VISIBLE_PLAYER&&CAN_SEE(mob,p))return TRUE;} return FALSE; }
static int ai_actor_vocalize(struct char_data *mob, time_t now)
{ const char *line; int i, tries=0, available=0; struct mob_ai_config*c; if(!mob||!mob->ai_config||!mob->ai_prof||!mob->ai_state||mob->ai_prof->communication!=AI_COMM_VOCALIZE||mob->ai_config->vocalization_count<=0||now<mob->ai_state->next_vocalization)return FALSE; c=mob->ai_config; if(mob->ai_state->vocal_room_vnum!=world[IN_ROOM(mob)].number){mob->ai_state->vocal_room_vnum=world[IN_ROOM(mob)].number;mob->ai_state->vocal_room_count=0;} if(!ai_actor_vocal_presence_ok(mob)||(c->vocal_room_limit&&mob->ai_state->vocal_room_count>=c->vocal_room_limit))return FALSE; for(i=0;i<c->vocalization_count;i++)if(c->vocalization[i]&&!(c->vocal_disabled_mask&(1U<<i)))available++; if(!available)return FALSE; do{i=rand_number(0,c->vocalization_count-1);}while((!c->vocalization[i]||(c->vocal_disabled_mask&(1U<<i))||(available>1&&i==mob->ai_state->last_vocalization_index))&&++tries<32); if(!c->vocalization[i]||(c->vocal_disabled_mask&(1U<<i)))return FALSE; line=c->vocalization[i]; act(line,TRUE,mob,0,0,TO_ROOM); mob->ai_state->last_vocalization=now; mob->ai_state->last_vocalization_index=i; mob->ai_state->vocal_room_count++; mob->ai_state->next_vocalization=now+rand_number(c->vocal_cooldown_min,c->vocal_cooldown_max); mob->ai_state->last_vocalization_result=TRUE; return TRUE; }
...
/* Legacy schedule compatibility: route execution ultimately calls perform_move(m,dir,1). */
/* The routine path cache is deliberately small (128 visited rooms/steps).  It
 * follows only normal open exits and is rebuilt after displacement or blockage. */
static int ai_routine_path(struct char_data *m, int target, int *route, int *length, char *why, size_t whylen)
{
  int queue[AI_ROUTINE_ROUTE_MAX], parent[AI_ROUTINE_ROUTE_MAX], pdir[AI_ROUTINE_ROUTE_MAX];
  int head=0, tail=0, found=-1, origin, zone, i, d;
  room_rnum goal;
  if (!m || IN_ROOM(m)==NOWHERE || (goal=real_room(target))==NOWHERE) { snprintf(why,whylen,"destination missing"); return FALSE; }
  if (ROOM_FLAGGED(goal,ROOM_NOMOB)||ROOM_FLAGGED(goal,ROOM_DEATH)) { snprintf(why,whylen,"prohibited destination room"); return FALSE; }
  origin=IN_ROOM(m); zone=world[origin].zone;
  if ((MOB_FLAGGED(m,MOB_STAY_ZONE)||m->ai_config->routine_boundary==AI_ROUTINE_CURRENT_ZONE) && world[goal].zone!=zone) { snprintf(why,whylen,"outside allowed boundary"); return FALSE; }
  queue[tail]=origin; parent[tail]=-1; pdir[tail]=-1; tail++;
  while(head<tail && tail<AI_ROUTINE_ROUTE_MAX) {
    int room=queue[head];
    if(room==goal){found=head;break;}
    for(d=0;d<DIR_COUNT;d++) if(world[room].dir_option[d] && world[room].dir_option[d]->to_room!=NOWHERE && !EXIT_FLAGGED(world[room].dir_option[d],EX_CLOSED)) {
      int to=world[room].dir_option[d]->to_room, seen=FALSE;
      if(ROOM_FLAGGED(to,ROOM_NOMOB)||ROOM_FLAGGED(to,ROOM_DEATH)) continue;
      if((MOB_FLAGGED(m,MOB_STAY_ZONE)||m->ai_config->routine_boundary==AI_ROUTINE_CURRENT_ZONE) && world[to].zone!=zone) continue;
      for(i=0;i<tail;i++) if(queue[i]==to){seen=TRUE;break;}
      if(seen) continue;
      queue[tail]=to; parent[tail]=head; pdir[tail]=d; tail++;
    } head++;
  }
  if(found<0){snprintf(why,whylen,tail>=AI_ROUTINE_ROUTE_MAX?"search limit reached":"no path found");return FALSE;}
  *length=0; while(parent[found]>=0){ if(*length>=AI_ROUTINE_ROUTE_MAX){snprintf(why,whylen,"route too long");return FALSE;} route[(*length)++]=pdir[found]; found=parent[found]; }
  for(i=0;i<*length/2;i++){d=route[i];route[i]=route[*length-1-i];route[*length-1-i]=d;} why[0]=0; return TRUE;
}
static int ai_routine_move_one(struct char_data *m,int target,time_t now)
{ struct ai_actor_state*s=m->ai_state; char why[sizeof(s->last_blocked_reason)]; int before;
  if(FIGHTING(m)||MOB_FLAGGED(m,MOB_SENTINEL)||IN_ROOM(m)==NOWHERE)return FALSE;
  if(s->routine_target_vnum!=target||s->routine_route_pos>=s->routine_route_len) { if(!ai_routine_path(m,target,s->routine_route,&s->routine_route_len,why,sizeof(why))){strlcpy(s->last_blocked_reason,why,sizeof(s->last_blocked_reason));s->routine_retry_at=now+10;return FALSE;} s->routine_target_vnum=target;s->routine_route_pos=0; }
  if(now<s->routine_next_move) return TRUE;
  before=IN_ROOM(m); s->routine_next_move=now+MAX(1,m->ai_config->movement_delay); s->last_schedule_move=now;
  if(!perform_move(m,s->routine_route[s->routine_route_pos],1)||IN_ROOM(m)==NOWHERE||IN_ROOM(m)==before){strlcpy(s->last_blocked_reason,"movement trigger prevented travel",sizeof(s->last_blocked_reason));s->routine_route_len=s->routine_route_pos=0;s->routine_retry_at=now+10;return FALSE;}
  s->routine_route_pos++; s->expected_room_vnum=world[IN_ROOM(m)].number; s->last_move_result=TRUE; return TRUE;
}

static int ai_routine_random_tick(struct char_data *m, time_t now)
{ struct mob_ai_config *c=m->ai_config; struct ai_actor_state *s=m->ai_state; int choices[AI_ROUTINE_DEST_MAX], n=0,i,pick,current;
  if(!c || (c->routine_mode!=AI_ROUTINE_RANDOM && c->routine_mode!=AI_ROUTINE_RANDOM_SCHEDULED) || FIGHTING(m) || MOB_FLAGGED(m,MOB_SENTINEL) || now<s->routine_wait_until || now<s->routine_retry_at) return FALSE;
  current=IN_ROOM(m)==NOWHERE?0:world[IN_ROOM(m)].number;
  if(s->routine_target_vnum && current==s->routine_target_vnum){s->routine_last_destination=current;s->routine_target_vnum=0;s->routine_route_len=s->routine_route_pos=0;s->routine_wait_until=now+rand_number(c->random_wait_min,c->random_wait_max);return TRUE;}
  if(!s->routine_target_vnum){for(i=0;i<c->random_destination_count;i++)if(c->random_destinations[i].enabled&&real_room(c->random_destinations[i].room_vnum)!=NOWHERE&&c->random_destinations[i].room_vnum!=current&&c->random_destinations[i].room_vnum!=s->routine_last_destination)choices[n++]=i; if(!n)for(i=0;i<c->random_destination_count;i++)if(c->random_destinations[i].enabled&&real_room(c->random_destinations[i].room_vnum)!=NOWHERE&&c->random_destinations[i].room_vnum!=current)choices[n++]=i; if(!n)return FALSE; pick=choices[rand_number(0,n-1)];s->routine_target_vnum=c->random_destinations[pick].room_vnum;s->routine_purpose=1;}
  if(!ai_routine_move_one(m,s->routine_target_vnum,now)){s->routine_target_vnum=0;s->routine_route_len=s->routine_route_pos=0;s->routine_retry_at=now+10;} return TRUE;
}

/* Whole game hours are the canonical authoring precision.  A game hour lasts
 * SECS_PER_MUD_HOUR real seconds, so route_steps * movement_delay is compared
 * directly to the next occurrence before travel begins. */
static int ai_routine_next_schedule(struct mob_ai_config *c, int day, int hour, int *days_ahead)
{
...
int ai_actor_tick_with_context(struct char_data *mob, time_t now, struct mob_behavior_pulse_context *ctx)
{
  ...
  npc_ai_build_profile(mob, &p);
  if (ctx && !mob_behavior_domain_available_to_ai(ctx, LBD_COMBAT_INIT)) { ctx->ai_result.domains_blocked |= LBD_COMBAT_INIT; }
  else if (ai_actor_combat_tick(mob, now)) { mob->ai_state->last_tick_result=AI_TICK_EXCLUSIVE; if (ctx) { ctx->ai_result.initiated_combat=TRUE; ctx->ai_result.consumed_pulse=TRUE; mob_behavior_mark_action(&ctx->ai_result, LBD_COMBAT_INIT); } return TRUE; }
  if (ctx && (!mob_behavior_domain_available_to_ai(ctx, LBD_ROUTINE) || !mob_behavior_domain_available_to_ai(ctx, LBD_MOVEMENT))) { ctx->ai_result.domains_blocked |= LBD_ROUTINE|LBD_MOVEMENT; }
  else if (ai_routine_schedule_tick(mob, now)) { mob->ai_state->last_tick_result=AI_TICK_EXCLUSIVE; if (ctx) { ctx->ai_result.moved=TRUE; ctx->ai_result.consumed_pulse=TRUE; mob_behavior_mark_action(&ctx->ai_result, LBD_ROUTINE|LBD_MOVEMENT); } return TRUE; }
  if (ctx && (!mob_behavior_domain_available_to_ai(ctx, LBD_ROUTINE) || !mob_behavior_domain_available_to_ai(ctx, LBD_POSTURE))) { ctx->ai_result.domains_blocked |= LBD_ROUTINE|LBD_POSTURE; }
  else if (ai_actor_schedule_tick(mob, now)) { mob->ai_state->last_tick_result=AI_TICK_EXCLUSIVE; if (ctx) { ctx->ai_result.changed_posture=TRUE; ctx->ai_result.consumed_pulse=TRUE; mob_behavior_mark_action(&ctx->ai_result, LBD_ROUTINE|LBD_POSTURE|LBD_MOVEMENT); } return TRUE; }
  if (ctx && (!mob_behavior_domain_available_to_ai(ctx, LBD_ROUTINE) || !mob_behavior_domain_available_to_ai(ctx, LBD_MOVEMENT))) { ctx->ai_result.domains_blocked |= LBD_ROUTINE|LBD_MOVEMENT; }
  else if (ai_routine_random_tick(mob, now)) { mob->ai_state->last_tick_result=AI_TICK_EXCLUSIVE; if (ctx) { ctx->ai_result.moved=TRUE; ctx->ai_result.consumed_pulse=TRUE; mob_behavior_mark_action(&ctx->ai_result, LBD_ROUTINE|LBD_MOVEMENT); } return TRUE; }
  mob->ai_state->last_idle_action=ai_actor_brain_think(mob, now);
  if (ctx && !mob_behavior_domain_available_to_ai(ctx, LBD_MOVEMENT)) { ctx->ai_result.domains_blocked |= LBD_MOVEMENT; }
  else if (ai_actor_random_move(mob, now)) { mob->ai_state->last_idle_action=AI_IDLE_MOVE_RANDOM; mob->ai_state->last_tick_result=AI_TICK_ACTED; if (ctx) { ctx->ai_result.moved=TRUE; mob_behavior_mark_action(&ctx->ai_result, LBD_MOVEMENT); } return TRUE; }
  if (mob->ai_prof->communication == AI_COMM_VOCALIZE) {
    if (ctx && !mob_behavior_domain_available_to_ai(ctx, LBD_AMBIENT_SPEECH)) { ctx->ai_result.domains_blocked |= LBD_AMBIENT_SPEECH; }
    else if (ai_actor_vocalize(mob, now)) { mob->ai_state->last_idle_action=AI_IDLE_VOCALIZE; mob->ai_state->last_tick_result=AI_TICK_ACTED; if (ctx) { ctx->ai_result.ambient_message=TRUE; mob_behavior_mark_action(&ctx->ai_result, LBD_AMBIENT_SPEECH); } return TRUE; }
    mob->ai_state->last_vocalization_result=FALSE;
  }
  if (mob->ai_prof->social == AI_SOCIAL_SILENT || mob->ai_prof->communication != AI_COMM_SPEAK) { mob->ai_state->last_tick_result=AI_TICK_IDLE; return TRUE; }
...
{
  return ai_actor_tick_with_context(mob, now, NULL);
}

...
int ai_schedule_add(struct mob_ai_config*c,const struct ai_schedule_entry*e){struct ai_schedule_entry z;if(!c||c->schedule_count>=AI_SCHEDULE_MAX)return FALSE;memset(&z,0,sizeof(z));if(e)z=*e;z.id=z.id?z.id:c->next_schedule_id++;z.day_mask=z.day_mask?z.day_mask:AI_DAY_MASK_ALL;z.max_attempts=z.max_attempts?z.max_attempts:3;c->schedules[c->schedule_count++]=z;return TRUE;}
...
int ai_patrol_add(struct mob_ai_config*c,const struct ai_patrol_route*r){struct ai_patrol_route z;if(!c||c->patrol_count>=AI_PATROL_MAX)return FALSE;memset(&z,0,sizeof(z));if(r)z=*r;z.id=z.id?z.id:c->next_patrol_id++;c->patrols[c->patrol_count++]=z;return TRUE;}
...
void ai_actor_schedule_preview(const struct mob_ai_config*c,int d,int h,char*out,size_t n){int i,a=ai_schedule_select(c,d,h);if(!out||!n)return;snprintf(out,n,"Schedule %s, game weekday %d hour %02d; active: %s\r\n",c&&c->schedule_enabled?"enabled":"disabled",d,h,a>=0?"selected":"none");if(!c)return;for(i=0;i<c->schedule_count;i++)ai_preview_add(out,n,"%c #%d %02d-%02d days %02x priority %d activity %d destination %d/%d\r\n",i==a?'*':' ',c->schedules[i].id,c->schedules[i].start_hour,c->schedules[i].end_hour,c->schedules[i].day_mask,c->schedules[i].priority,c->schedules[i].activity,c->schedules[i].destination,c->schedules[i].destination_value);}
void ai_actor_schedule_validate(const struct mob_ai_config*c,char*out,size_t n){int i,j;if(!out||!n)return;out[0]=0;if(!c){ai_preview_add(out,n,"ERROR: no schedule configuration.\r\n");return;}if(c->schedule_enabled&&!c->schedule_count)ai_preview_add(out,n,"ERROR: schedule enabled with no entries.\r\n");for(i=0;i<c->schedule_count;i++){const struct ai_schedule_entry*e=&c->schedules[i];if(e->start_hour<0||e->start_hour>23||e->end_hour<0||e->end_hour>23||!e->day_mask)ai_preview_add(out,n,"ERROR: invalid time/day in entry %d.\r\n",e->id);if(e->activity<0||e->activity>=AI_SCHEDULE_ACTIVITY_MAX||e->destination<0||e->destination>=AI_DESTINATION_MAX)ai_preview_add(out,n,"ERROR: invalid activity/destination in entry %d.\r\n",e->id);if(e->max_attempts<=0)ai_preview_add(out,n,"ERROR: zero attempts in entry %d.\r\n",e->id);for(j=i+1;j<c->schedule_count;j++)if(e->priority==c->schedules[j].priority&&ai_schedule_entries_overlap(e,&c->schedules[j]))ai_preview_add(out,n,"WARNING: equal-priority overlap %d/%d resolves by stable order.\r\n",e->id,c->schedules[j].id);}if(!*out)ai_preview_add(out,n,"Schedule validation: no errors or warnings.\r\n");}
...
static int ai_schedule_destination(struct char_data *m,const struct ai_schedule_entry *e)
...
static void ai_schedule_dialogue(struct char_data *m,int category,time_t now)
{ if(m->ai_prof && m->ai_prof->social!=AI_SOCIAL_SILENT && now-m->ai_state->last_spoke>=m->ai_config->speech_cooldown) ai_social_say(m,category,NULL,now); }
static void ai_schedule_action(struct char_data *m,int action,int category,time_t now)
{ if(action==AI_SCHEDULE_ACTION_SPEAK) ai_schedule_dialogue(m,category,now); else if(action==AI_SCHEDULE_ACTION_EMOTE) ai_social_emote(m,now); else if(action==AI_SCHEDULE_ACTION_SIT) do_sit(m,"",0,0); else if(action==AI_SCHEDULE_ACTION_REST) do_rest(m,"",0,0); else if(action==AI_SCHEDULE_ACTION_SLEEP && !FIGHTING(m)) do_sleep(m,"",0,0); else if(action==AI_SCHEDULE_ACTION_STAND) do_stand(m,"",0,0); else if(action==AI_SCHEDULE_ACTION_WAKE) do_wake(m,"",0,0); }
void ai_actor_schedule_interrupt(struct char_data *m,int reason,long actor_id,time_t now)
{ struct ai_actor_state*s; (void)actor_id; if(!m||(s=m->ai_state)==NULL||!s->active_schedule_id)return; s->schedule_interrupted=TRUE; s->schedule_reason=reason; s->schedule_state=AI_SCHED_INTERRUPTED; s->schedule_wait_until=now; }
static void ai_schedule_failure(struct char_data *m,struct ai_schedule_entry *e,time_t now)
{ struct ai_actor_state*s=m->ai_state; int policy=e->failure_policy; s->schedule_failures++; s->schedule_state=AI_SCHED_FAILED; ai_schedule_dialogue(m,AI_DIALOGUE_SCHEDULE_FAILURE,now); if(s->schedule_attempts<e->max_attempts) { s->schedule_retry_at=now+MAX(1,e->wait_duration); return; } if(policy==AI_FAILURE_SKIP) { s->schedule_skipped_id=e->id; s->active_schedule_id=0; } else if(policy==AI_FAILURE_RESTART) { s->schedule_waypoint=0;s->patrol_direction=1;s->schedule_attempts=0;s->schedule_retry_at=now+MAX(1,e->wait_duration); } else if(policy==AI_FAILURE_FALLBACK) { s->schedule_destination_vnum=m->ai_config->fallback_room_vnum;s->schedule_retry_at=now+MAX(1,e->wait_duration); } else if(policy==AI_FAILURE_ABORT||policy==AI_FAILURE_DISABLE_UNTIL_CHANGE) { s->schedule_skipped_id=e->id;s->active_schedule_id=0;s->schedule_state=AI_SCHED_ABORTED; } else s->schedule_retry_at=now+MAX(1,e->wait_duration); }
static int ai_actor_schedule_tick(struct char_data *m,time_t now)
{ struct mob_ai_config*c=m->ai_config; struct ai_actor_state*s=m->ai_state; int day=((35*time_info.month)+(time_info.day+1))%7,idx,dest,dir; struct ai_schedule_entry*e; struct ai_patrol_route*r=NULL;
  if(!c||!c->schedule_enabled)return FALSE;
  if(FIGHTING(m)){ai_actor_schedule_interrupt(m,1,0,now);return TRUE;}
  if(s->expected_room_vnum&&IN_ROOM(m)!=NOWHERE&&world[IN_ROOM(m)].number!=s->expected_room_vnum){ai_actor_schedule_interrupt(m,2,0,now);s->expected_room_vnum=world[IN_ROOM(m)].number;return TRUE;}
  idx=ai_schedule_select(c,day,time_info.hours); if(idx<0||c->schedules[idx].id==s->schedule_skipped_id)return FALSE; e=&c->schedules[idx];
  if(s->schedule_interrupted){ if(!c->resume_after_interrupt||e->interruption_policy==AI_INTERRUPT_SKIP||e->interruption_policy==AI_INTERRUPT_ABORT){s->schedule_skipped_id=e->id;s->active_schedule_id=0;s->schedule_interrupted=FALSE;return FALSE;} if(e->interruption_policy==AI_INTERRUPT_RESTART){s->schedule_waypoint=0;s->patrol_direction=1;} if(e->interruption_policy==AI_INTERRUPT_FALLBACK){s->schedule_destination_vnum=c->fallback_room_vnum;} s->schedule_interrupted=FALSE;s->schedule_state=AI_SCHED_RESUMING; }
  if(s->active_schedule_id!=e->id){s->previous_schedule_id=s->active_schedule_id;s->active_schedule_id=e->id;s->schedule_started_at=now;s->schedule_failures=s->schedule_attempts=0;s->schedule_wait_until=s->schedule_retry_at=0;s->schedule_waypoint=0;s->patrol_direction=1;s->schedule_state=AI_SCHED_SELECTED;}
  s->schedule_wander_suppressed=TRUE; if(s->schedule_retry_at>now||s->schedule_wait_until>now)return TRUE;
  if(e->max_travel_time>0&&now-s->schedule_started_at>e->max_travel_time){ai_schedule_failure(m,e,now);return TRUE;}
  if(e->activity==AI_SCHEDULE_SLEEP||e->activity==AI_SCHEDULE_REST){ if(!FIGHTING(m)&&GET_POS(m)!= (e->activity==AI_SCHEDULE_SLEEP?POS_SLEEPING:POS_RESTING)){ai_schedule_dialogue(m,e->activity==AI_SCHEDULE_SLEEP?AI_DIALOGUE_SLEEP:AI_DIALOGUE_WORK,now); ai_schedule_action(m,e->activity==AI_SCHEDULE_SLEEP?AI_SCHEDULE_ACTION_SLEEP:AI_SCHEDULE_ACTION_REST,0,now);} s->schedule_state=AI_SCHED_ACTIVE;return TRUE; }
  if(GET_POS(m)==POS_SLEEPING){do_wake(m,"",0,0);ai_schedule_dialogue(m,AI_DIALOGUE_WAKE,now);return TRUE;} if(GET_POS(m)<POS_STANDING){do_stand(m,"",0,0);return TRUE;}
  if(e->activity==AI_SCHEDULE_PATROL){r=ai_schedule_route(c,e->route_id);if(!r||!r->enabled||!r->waypoint_count){ai_schedule_failure(m,e,now);return TRUE;} dest=r->waypoints[s->schedule_waypoint].room_vnum;} else dest=ai_schedule_destination(m,e);
  if(!dest||real_room(dest)==NOWHERE){ai_schedule_failure(m,e,now);return TRUE;} s->schedule_destination_vnum=dest;
  if(IN_ROOM(m)!=NOWHERE&&world[IN_ROOM(m)].number==dest){ if(r){ai_schedule_action(m,r->waypoints[s->schedule_waypoint].arrival_action,AI_DIALOGUE_PATROL,now);s->schedule_wait_until=now+MAX(0,r->waypoints[s->schedule_waypoint].wait_duration);s->schedule_waypoint=ai_patrol_advance(r,s->schedule_waypoint,s->patrol_direction,&s->patrol_direction);if(s->schedule_waypoint<0){s->schedule_state=AI_SCHED_COMPLETED;s->active_schedule_id=0;}else s->schedule_state=AI_SCHED_WAITING_WAYPOINT;} else {s->schedule_state=AI_SCHED_ACTIVE;if(e->activity==AI_SCHEDULE_WORK)ai_schedule_dialogue(m,AI_DIALOGUE_WORK,now);if(e->activity==AI_SCHEDULE_GUARD)ai_schedule_dialogue(m,AI_DIALOGUE_GUARD,now);} return TRUE; }
  if(MOB_FLAGGED(m,MOB_SENTINEL)||m->master||IN_ROOM(m)==NOWHERE){ai_schedule_failure(m,e,now);return TRUE;}
  if(s->schedule_state!=AI_SCHED_PREPARING_DEPARTURE){ai_schedule_action(m,e->departure_action,AI_DIALOGUE_SCHEDULE_DEPARTURE,now);s->last_departure_action=e->departure_action;s->schedule_state=AI_SCHED_PREPARING_DEPARTURE;return TRUE;}
  for(dir=0;dir<DIR_COUNT;dir++)if(EXIT(m,dir)&&EXIT(m,dir)->to_room!=NOWHERE&&world[EXIT(m,dir)->to_room].number==dest)break;
  if(dir==DIR_COUNT||ROOM_FLAGGED(EXIT(m,dir)->to_room,ROOM_NOMOB)||ROOM_FLAGGED(EXIT(m,dir)->to_room,ROOM_DEATH)||(MOB_FLAGGED(m,MOB_STAY_ZONE)&&world[EXIT(m,dir)->to_room].zone!=world[IN_ROOM(m)].zone)){s->schedule_attempts++;ai_schedule_failure(m,e,now);return TRUE;}
  s->expected_room_vnum=dest;s->last_schedule_move=now;s->schedule_state=AI_SCHED_TRAVELING;if(!perform_move(m,dir,1)||IN_ROOM(m)==NOWHERE||world[IN_ROOM(m)].number!=dest){s->schedule_attempts++;ai_schedule_failure(m,e,now);}else{s->schedule_failures=s->schedule_attempts=0;ai_schedule_action(m,e->arrival_action,AI_DIALOGUE_SCHEDULE_ARRIVAL,now);s->last_arrival_action=e->arrival_action;s->schedule_state=AI_SCHED_ARRIVED;} return TRUE;
}
...
static void ai_actor_threat_event(struct char_data *mob, struct char_data *actor, int base, int confidence, time_t now) {
  ...
  if(s->type>=AI_THREAT_WARN && s->type!=AI_THREAT_IGNORE) ai_actor_schedule_interrupt(mob,s->type,GET_IDNUM(actor),now);

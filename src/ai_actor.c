...
static void ai_actor_clean_sentence(char *s);
...
static enum ai_rx_intent ai_rx_classify_intent(enum ai_event_type type, const char *norm_text)
{
  ...
  if (ai_text_has_sub_ci(norm_text, "help") || ai_text_has_sub_ci(norm_text, "save me")) return RX_INTENT_REQUEST_HELP;
  if (ai_text_has_sub_ci(norm_text, "heal") || ai_text_has_sub_ci(norm_text, "cure")) return RX_INTENT_REQUEST_HEAL;
  if (ai_text_has_sub_ci(norm_text, "buy") || ai_text_has_sub_ci(norm_text, "sell") || ai_text_has_sub_ci(norm_text, "trade")) return RX_INTENT_REQUEST_TRADE;
  if (ai_text_has_sub_ci(norm_text, "train") || ai_text_has_sub_ci(norm_text, "practice")) return RX_INTENT_REQUEST_TRAIN;
  if (ai_text_has_sub_ci(norm_text, "sorry") || ai_text_has_sub_ci(norm_text, "forgive")) return RX_INTENT_APOLOGY;
  if (ai_text_has_sub_ci(norm_text, "praise") || ai_text_has_sub_ci(norm_text, "thank")) return RX_INTENT_PRAISE;
  if (ai_text_has_sub_ci(norm_text, "kill") || ai_text_has_sub_ci(norm_text, "die") || ai_text_has_sub_ci(norm_text, "murder")) return RX_INTENT_THREATEN;
  if (ai_text_has_sub_ci(norm_text, "idiot") || ai_text_has_sub_ci(norm_text, "stupid") || ai_text_has_sub_ci(norm_text, "fool")) return RX_INTENT_INSULT;
  if (ai_text_has_sub_ci(norm_text, "steal") || ai_text_has_sub_ci(norm_text, "rob") || ai_text_has_sub_ci(norm_text, "mug")) return RX_INTENT_LOOT_THEFT;
  if (ai_text_has_sub_ci(norm_text, "pray") || ai_text_has_sub_ci(norm_text, "bless") || ai_text_has_sub_ci(norm_text, "mercy")) return RX_INTENT_CONFESSION_PRAYER;
  if (strchr(norm_text, '?')) return RX_INTENT_QUESTION;
  if (ai_text_has_sub_ci(norm_text, "hello") || ai_text_has_sub_ci(norm_text, "hi ")) return RX_INTENT_GREETING;
...
static void ai_actor_build_room_context(struct room_data *room, const char *norm_text, enum ai_rx_intent intent, struct ai_room_context *out)
{
  ...
  if (intent == RX_INTENT_DAMAGE_EVENT || intent == RX_INTENT_THREATEN || (norm_text && (ai_text_has_sub_ci(norm_text, "bleed") || ai_text_has_sub_ci(norm_text, "slashes") || ai_text_has_sub_ci(norm_text, "hits")))) out->bloodshed_recent = 1;
  if (intent == RX_INTENT_LOOT_THEFT || (norm_text && (ai_text_has_sub_ci(norm_text, "steal") || ai_text_has_sub_ci(norm_text, "rob")))) out->law_breaking_recent = 1;
...
static void ai_rx_build_moral_context(struct char_data *mob, struct char_data *actor, enum ai_rx_intent intent, const struct ai_room_context *rc, struct ai_moral_context *mc)
{
  ...
  mc->victim_is_good = actor ? IS_GOOD(actor) : 0;
  mc->aggressor_is_evil = actor ? IS_EVIL(actor) : 0;
  mc->victim_is_innocent = (intent == RX_INTENT_REQUEST_HELP || intent == RX_INTENT_REQUEST_HEAL);
  mc->player_is_known = 1;
  mc->urgency = 10;
  if (rc) mc->urgency += rc->threat_level * 20 + (rc->bloodshed_recent ? 20 : 0) + (rc->law_breaking_recent ? 20 : 0);
  if (intent == RX_INTENT_REQUEST_HELP || intent == RX_INTENT_REQUEST_HEAL || intent == RX_INTENT_PLAYER_DOWN) mc->urgency += 25;
  if (mob && IS_GOOD(mob)) mc->urgency += 8;
  if (mob && IS_EVIL(mob)) mc->urgency -= 8;
  mc->urgency = MAX(0, MIN(100, mc->urgency));
}
...
static int ai_rx_guess_ext_archetype(struct char_data *mob, int role, int style)
{
  ...
  if (role == ROLE_GUARD || role == ROLE_BOSS) return AI_ARCHX_GUARD_LAW;
  if (role == ROLE_MERCHANT && style == 1) return AI_ARCHX_CIVILIAN;
  ...
  if (ai_text_has_sub_ci(sd, "paladin") || ai_text_has_sub_ci(sd, "templar")) return AI_ARCHX_PALADIN_HOLY;
  if (ai_text_has_sub_ci(sd, "healer") || ai_text_has_sub_ci(sd, "priest")) return AI_ARCHX_HEALER_SUPPORT;
  if (ai_text_has_sub_ci(sd, "instructor") || ai_text_has_sub_ci(sd, "trainer") || MOB_FLAGGED(mob, MOB_GUILD_MASTER)) return AI_ARCHX_INSTRUCTOR;
  if (ai_text_has_sub_ci(sd, "commander") || ai_text_has_sub_ci(sd, "captain")) return AI_ARCHX_COMMANDER;
  if (ai_text_has_sub_ci(sd, "wolf") || ai_text_has_sub_ci(sd, "bear") || ai_text_has_sub_ci(sd, "beast")) return AI_ARCHX_BEAST_ANIMAL;
  if (ai_text_has_sub_ci(sd, "skeleton") || ai_text_has_sub_ci(sd, "zombie")) return AI_ARCHX_UNDEAD_MINDLESS;
  if (ai_text_has_sub_ci(sd, "spirit") || ai_text_has_sub_ci(sd, "wraith") || ai_text_has_sub_ci(sd, "ghost")) return AI_ARCHX_SPIRIT_ETHEREAL;
...
static unsigned long ai_rx_seed_mob(struct char_data *mob)
{
  ...
  if (!mob) return s;
  s = ai_hash_mix(s, (unsigned long)GET_MOB_VNUM(mob));
  s = ai_hash_mix(s, (unsigned long)GET_IDNUM(mob));
  s = ai_hash_mix(s, (unsigned long)GET_ALIGNMENT(mob));
  return s;
}
...
static void ai_rx_personality(struct char_data *mob, int ext_arch, struct ai_personality_ext *out)
{
  ...
  out->bravery = 35 + (int)(seed % 56);
  out->empathy = 30 + (int)((seed >> 3) % 60);
  out->honor = 30 + (int)((seed >> 7) % 60);
  out->aggression = 20 + (int)((seed >> 11) % 65);
  out->lawfulness = 20 + (int)((seed >> 15) % 70);
  out->piety = 20 + (int)((seed >> 19) % 70);
  out->wit = 20 + (int)((seed >> 5) % 70);
  out->patience = 20 + (int)((seed >> 9) % 70);
  out->fearfulness = 100 - out->bravery;
  if (mob && IS_GOOD(mob)) { out->empathy += 15; out->honor += 12; out->piety += 10; out->aggression -= 10; }
  if (mob && IS_EVIL(mob)) { out->aggression += 18; out->wit += 8; out->empathy -= 12; out->honor -= 10; }
  if (ext_arch == AI_ARCHX_PALADIN_HOLY || ext_arch == AI_ARCHX_COMMANDER) { out->lawfulness += 25; out->honor += 20; }
  if (ext_arch == AI_ARCHX_HEALER_SUPPORT) { out->empathy += 25; out->piety += 15; }
  if (ext_arch == AI_ARCHX_BANDIT_CRIMINAL) { out->aggression += 20; out->lawfulness -= 25; }
  if (ext_arch == AI_ARCHX_BEAST_ANIMAL) { out->wit -= 20; out->patience -= 10; }
  if (ext_arch == AI_ARCHX_UNDEAD_MINDLESS) { out->empathy = 0; out->piety = 0; out->patience = 5; }
...
void ai_actor_on_room_event(struct char_data *mob, enum ai_event_type type, struct char_data *actor, const char *text)
{
  ...
  enum ai_rx_intent rx_intent = RX_INTENT_NONE;
...
#define AI_EVT_RETURN(_reason) do { \
  ...
  rx_intent = ai_rx_classify_intent(type, normalized);
  ai_actor_build_room_context(&world[IN_ROOM(mob)], normalized, rx_intent, &room_ctx);
  ai_rx_build_moral_context(mob, actor, rx_intent, &room_ctx, &moral_ctx);
  if (type == AI_EVENT_PLAYER_SAY) {
  ...
  ai_rel_apply_event(mob, GET_IDNUM(actor), rx_intent, now);
...

  {
    ...
    if (ai_rx_decide_override_line(mob, actor, normalized, rx_intent, &room_ctx, &moral_ctx, rx_override, sizeof(rx_override), &rx_force_emote, now))
      line = rx_override;
    ...
    ai_set_last_speech_meta(mob, pool, reason);
    {
      ...
      ai_actor_clean_sentence(finalbuf);
  ...
  ai_debug_log("AI_EVT vnum=%d role=%s goal=%s mbti=%s tb=%s arch=%s arc=%s ex=%d attn=%.2f susp=%.2f action=%s cd=%.0f rx_int=%d threat=%d urg=%d align=%d",
               GET_MOB_VNUM(mob), ai_role_name_local(mob->ai_prof->role), ai_goal_name(goal ? goal->type : AI_GOAL_NONE), ai_mbti_string(ai_voice_profile_get(mob)),
               ai_time_bucket_name(ctx.time_bucket), ai_arch_name(sr ? sr->archetype : AI_ARCH_UNKNOWN), ai_arc_name(sr ? sr->arc : AI_ARC_STRANGER),
               sr ? sr->exchange_count : 0, attention_score, suspicion, ai_action_name(best_action), cooldown_remaining, rx_intent, room_ctx.threat_level, moral_ctx.urgency, GET_ALIGNMENT(mob));

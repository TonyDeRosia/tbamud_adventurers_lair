...
int ai_actor_tick(struct char_data *mob, time_t now)
{
  ...
  if (ai_routine_schedule_tick(mob, now)) { mob->ai_state->last_tick_result=AI_TICK_EXCLUSIVE; return TRUE; }
  if (ai_actor_schedule_tick(mob, now)) { mob->ai_state->last_tick_result=AI_TICK_EXCLUSIVE; return TRUE; }
  if (ai_routine_random_tick(mob, now)) { mob->ai_state->last_tick_result=AI_TICK_EXCLUSIVE; return TRUE; }
  ...
  if (mob->ai_prof->communication == AI_COMM_VOCALIZE) {
    if (ai_actor_vocalize(mob, now)) { mob->ai_state->last_idle_action=AI_IDLE_VOCALIZE; mob->ai_state->last_tick_result=AI_TICK_ACTED; return TRUE; }

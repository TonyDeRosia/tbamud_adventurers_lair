...
{ char clean[AI_VOCALIZATION_LINE_MAX], *p; if(!c||i<0||i>=AI_VOCALIZATION_MAX_LINES||!line)return FALSE; strlcpy(clean,line,sizeof(clean)); for(p=clean;*p;p++) if(*p=='\r'||*p=='\n')*p=' '; skip_spaces(&p); if(!*p)return FALSE; free(c->vocalization[i]); c->vocalization[i]=strdup(p); if(i>=c->vocalization_count)c->vocalization_count=i+1; return c->vocalization[i]!=NULL; }
int mob_ai_vocalization_delete(struct mob_ai_config *c, int i)
{ int n; if(!c||i<0||i>=c->vocalization_count)return FALSE; free(c->vocalization[i]); for(n=i;n+1<c->vocalization_count;n++)c->vocalization[n]=c->vocalization[n+1]; c->vocalization[--c->vocalization_count]=NULL; return TRUE; }
int mob_ai_vocalization_move(struct mob_ai_config *c, int from, int to)
{ char *line; if(!c||from<0||to<0||from>=c->vocalization_count||to>=c->vocalization_count)return FALSE; line=c->vocalization[from]; if(from<to)for(;from<to;from++)c->vocalization[from]=c->vocalization[from+1];else for(;from>to;from--)c->vocalization[from]=c->vocalization[from-1];c->vocalization[to]=line;return TRUE; }

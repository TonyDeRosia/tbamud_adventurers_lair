...
{ const char *line; int i; if(!mob||!mob->ai_config||!mob->ai_prof||!mob->ai_state||mob->ai_prof->communication!=AI_COMM_VOCALIZE||mob->ai_config->vocalization_count<=0||now<mob->ai_state->next_vocalization)return FALSE; i=rand_number(0,mob->ai_config->vocalization_count-1); line=mob->ai_config->vocalization[i]; if(!line||!*line)return FALSE; act(line,TRUE,mob,0,0,TO_ROOM); mob->ai_state->last_vocalization=now; mob->ai_state->next_vocalization=now+MAX(10,mob->ai_prof->room_talk_cooldown_secs); mob->ai_state->last_vocalization_result=TRUE; return TRUE; }

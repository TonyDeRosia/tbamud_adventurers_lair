...
static int ai_schedule_destination(struct char_data *m,const struct ai_schedule_entry *e){struct mob_ai_config*c=m->ai_config;switch(e->destination){case AI_DEST_CURRENT_ROOM:return IN_ROOM(m)!=NOWHERE?world[IN_ROOM(m)].number:0;case AI_DEST_ROOM_VNUM:return e->destination_value;case AI_DEST_HOME:return c->home_room_vnum;case AI_DEST_WORK:return c->work_room_vnum;case AI_DEST_GUARD:return c->guard_room_vnum;case AI_DEST_SPAWN:return GET_MOB_VNUM(m);default:return 0;}}
static int ai_actor_schedule_tick(struct char_data *m,time_t now){struct mob_ai_config*c=m->ai_config;struct ai_actor_state*s=m->ai_state;int day=((35*time_info.month)+(time_info.day+1))%7,idx,dest,dir;struct ai_schedule_entry*e;if(!c||!c->schedule_enabled)return FALSE;if(FIGHTING(m)||!AWAKE(m)){s->schedule_interrupted=TRUE;return TRUE;}idx=ai_schedule_select(c,day,time_info.hours);if(idx<0)return FALSE;e=&c->schedules[idx];if(s->active_schedule_id!=e->id){s->previous_schedule_id=s->active_schedule_id;s->active_schedule_id=e->id;s->schedule_started_at=now;s->schedule_failures=0;s->schedule_wait_until=0;s->schedule_waypoint=0;s->patrol_direction=1;}if(s->schedule_wait_until>now)return TRUE;if(e->activity==AI_SCHEDULE_SLEEP||e->activity==AI_SCHEDULE_REST){if(GET_POS(m)==POS_STANDING)GET_POS(m)=e->activity==AI_SCHEDULE_SLEEP?POS_SLEEPING:POS_RESTING;return TRUE;}if(GET_POS(m)<POS_STANDING){GET_POS(m)=POS_STANDING;return TRUE;}if(e->activity==AI_SCHEDULE_PATROL){struct ai_patrol_route*r=ai_schedule_route(c,e->route_id);if(!r||!r->enabled||!r->waypoint_count)return TRUE;dest=r->waypoints[s->schedule_waypoint].room_vnum;if(IN_ROOM(m)!=NOWHERE&&world[IN_ROOM(m)].number==dest){s->schedule_wait_until=now+r->waypoints[s->schedule_waypoint].wait_duration;s->schedule_waypoint=ai_patrol_advance(r,s->schedule_waypoint,s->patrol_direction,&s->patrol_direction);if(s->schedule_waypoint<0)s->active_schedule_id=0;return TRUE;}}else{dest=ai_schedule_destination(m,e);if(!dest||real_room(dest)==NOWHERE)return TRUE;if(IN_ROOM(m)!=NOWHERE&&world[IN_ROOM(m)].number==dest)return TRUE;}if(MOB_FLAGGED(m,MOB_SENTINEL)||m->master)return TRUE;if(IN_ROOM(m)==NOWHERE)return TRUE;dir=-1;for(int d=0;d<DIR_COUNT;d++)if(EXIT(m,d)&&EXIT(m,d)->to_room!=NOWHERE&&world[EXIT(m,d)->to_room].number==dest){dir=d;break;}if(dir<0){s->schedule_failures++;return TRUE;}if(ROOM_FLAGGED(EXIT(m,dir)->to_room,ROOM_NOMOB)||ROOM_FLAGGED(EXIT(m,dir)->to_room,ROOM_DEATH)||(MOB_FLAGGED(m,MOB_STAY_ZONE)&&world[EXIT(m,dir)->to_room].zone!=world[IN_ROOM(m)].zone)){s->schedule_failures++;return TRUE;}s->expected_room_vnum=dest;s->last_schedule_move=now;if(!perform_move(m,dir,1))s->schedule_failures++;return TRUE;}

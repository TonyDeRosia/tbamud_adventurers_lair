...
int ai_brain_can_speak(const struct char_data *mob) { (void)mob; return 1; }

void ai_actor_brain_init(struct char_data *mob) { (void)mob; }
void ai_actor_brain_free(struct char_data *mob) { (void)mob; }
int ai_actor_brain_think(struct char_data *mob, time_t now) { (void)mob; (void)now; return FALSE; }
...
void ai_actor_brain_show_state(struct char_data *viewer, struct char_data *mob)
{
  ...
  send_to_char(viewer, "AI brain authority is removed for humanoid social behavior.\\r\\n");

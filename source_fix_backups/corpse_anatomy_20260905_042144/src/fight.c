...
static const struct body_part_drop body_part_drops[] = {
  ...
  { BODY_PART_MIDLEG_LEFT, 7 },
  { BODY_PART_MIDLEG_RIGHT, 7 },
  { BODY_PART_REARLEG_LEFT, 7 },
  { BODY_PART_REARLEG_RIGHT, 7 },
  { BODY_PART_FIN_LEFT, 7 },
  { BODY_PART_FIN_RIGHT, 7 },
  { BODY_PART_BRANCH, 7 },
  { BODY_PART_ROOT, 7 },
  { BODY_PART_CLAW_LEFT, 7 },
  { BODY_PART_CLAW_RIGHT, 7 }
};
 ...
 * stale character pointer in a round-participant list. */
static void mark_combat_prompt_participant(struct char_data *ch, unsigned long round)
{
  if (ch && !IS_NPC(ch) && ch->desc && STATE(ch->desc) == CON_PLAYING)
    ch->desc->combat_prompt_round = round;
}
...
static void queue_combat_round_prompts(unsigned long round)
{
  ...
  for (d = descriptor_list; d; d = d->next) {
    if (d->combat_prompt_round != round || STATE(d) != CON_PLAYING ||
        !d->character || IS_NPC(d->character) || d->showstr_count || d->str)
      continue;
    queue_prompt(d);
  }
}
...
void perform_violence(void)
{
  ...
  unsigned long prompt_round;

  if (++combat_prompt_round == 0)
    ++combat_prompt_round;
  ...
  for (ch = combat_list; ch; ch = next_combat_list) {
    ...
    /* Mark both direct combatants before any hit, trigger, death, flee, or
     * extraction path can clear FIGHTING().  The actual prompt is emitted
     * once, after every fighter's synchronous round output is complete. */
    mark_combat_prompt_participant(ch, prompt_round);
    mark_combat_prompt_participant(FIGHTING(ch), prompt_round);
  ...
  queue_combat_round_prompts(prompt_round);
}

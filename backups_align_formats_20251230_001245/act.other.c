...
{
  int i, lvl, last_lvl = -1;
  int col = 0;
  int cls = GET_CLASS(ch);
  const char *cont = "           "; /* matches "Level 101: " */
  ...
  for (i = 1; i <= TOP_SPELL_DEFINE; i++) {
    ...
    {
      char cell[256];
      snprintf(cell, sizeof(cell), "%s [%d%%]", spell_info[i].name, GET_SKILL(ch, i));
